...
Status: Committed at `e6c9189` (N-2b) → `9ae139d` (N-2c, HEAD-preflight correction) → `3732721` (N-2d, connectivity-preflight runbook update). N-2e is the N-3 preflight tightening phase: tightens §4(vii) into four PASS/FAIL categories (DNS / TLS / auth / target-identity); adds §4(viii) concurrent-runner confirmation; adds §4(ix) no-local-`.env`-shadow confirmation; expands §6 verification into full column / index definition checks; disambiguates §7 exit 1 vs exit 2; adds §7 HALT-on-no-applying-008 + HALT-on-all-migrations-applied rules; adds four new §8 risk-table rows (P1 concurrent runner; P1 dotenv shadow; P1 stale Railway CLI / project / service auth; P2 runner version drift); updates §9 gate-separation wording; adds the HEAD-placeholder requirement to the §5 approval block. **One N-3 attempt was made and HALTED before any SQL executed** — see §1.1 N-3 attempt history below. **Migration 008 remains NOT applied to production.** N-3 remains blocked pending Codex docs-only PASS on this N-2e update, fresh Codex N-3 preflight PASS on the updated runbook, fresh Victor explicit in-session production-action approval naming the exact current HEAD hash, all nine pre-flight checks (i)–(ix) passing at execution time, and target Railway service / production database confirmation without exposing secrets.
...
>
> **If any check (i)–(ix) differs from expected, HALT and re-request Codex review and operator approval before proceeding.**
...
| **P2** | **Runner version drift between approved HEAD and Railway-attached execution** — the `package.json` / `node_modules` / Node version in the Railway-attached execution context may differ from the local approved HEAD, causing the runner file at the approved HEAD to behave differently at runtime. | **Pre-flight (i) + (iii) already constrain the runner SOURCE FILE to the approved HEAD via `git rev-parse HEAD` + `git diff --name-status HEAD` empty. Operator additionally confirms (without exposing secrets) that the Railway-attached execution context resolves `scripts/run-migrations.js` and its `node_modules` / Node version from the approved HEAD's runtime — i.e., that the Railway service has been deployed from the approved HEAD or is otherwise running the approved-HEAD runtime — before invoking the runner.** *(N-2e addition.)* |
...
4. **All pre-flight checks (§4 (i)–(ix)) pass** at the moment of N-3 execution, including the (vii) connectivity preflight (DNS / TLS / auth / target-identity categories), (viii) concurrent-runner confirmation, and (ix) no-local-`.env`-shadow confirmation. If any check fails, HALT.
