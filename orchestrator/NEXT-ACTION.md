...
The next allowed action after this N-2m doc draft is: (a) Codex docs-only review of the runbook diff + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit N-2m; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2m commit, **N-3 remains halted/blocked** behind the five-condition gate, with both prior Victor approvals (`9ae139d` and `3138e7f`) already consumed. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2m commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h / N-2i / N-2j / N-2k / N-2l / N-2m / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
...
> **N-2m Codex docs-only review of the second-N-3-HALT documentation + same-session connectivity preflight tightening runbook update, then explicit operator approval before any N-2m commit; N-3 remains halted/blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2m, fresh Codex N-3 preflight PASS, fresh Victor approval naming a NEW HEAD — both `9ae139d` and `3138e7f` approvals are consumed, all eleven pre-flight checks (i)–(xi) PASS at execution time including the new same-session §4(vii) requirement, target confirmation without exposing secrets).**
...
- Before applying Migration 008: follow the current N-2m runbook gate — fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD (note: both prior approvals at `9ae139d` and `3138e7f` are consumed), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time (including the §4(vii) connectivity preflight performed in the EXACT SAME execution context that will invoke the runner with no context switch permitted between (vii) PASS and runner invocation, and the (a) DNS / (c) auth checks exercising the actual Node-process resolver path; and the §4(x) three-component runtime-source identity check verified in the same context as §4(vii)), target confirmation without exposing secrets, then the full §6 verification after runner exit.
...
For **Migration 008 production-application planning** (when authorized): the operator-driven sequence is — (1) satisfy the current N-2m runbook gate, including fresh Codex N-3 preflight PASS, fresh Victor approval naming a new HEAD (both prior approvals at `9ae139d` and `3138e7f` are consumed by HALTED attempts), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time (including the §4(vii) same-session / no-context-switch / actual-runner-DNS-path requirements added in N-2m and the §4(x) three-component runtime-source identity check verified in the same context as §4(vii)), and target confirmation without exposing secrets; (2) apply Migration 008 only under that explicit production-action approval; (3) complete the full §6 post-application verification; (4) document the application timestamp in `STATUS.md`. After application, D-5.12d/e production deployment is unblocked (still requires its own explicit operator authorization).
