...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 in progress (Phase ARC-4 — mode: DOCS-ONLY).**
  ...
  - [~] **ARC-4 — Next-Action Selector (Phase ARC-4 — mode: DOCS-ONLY) — IN PROGRESS.**
    - [x] New `orchestrator/NEXT-ACTION-SELECTOR.md` drafted as a safety-policy doc enforcing strict ordering of future Agent Avila phases.
    - [x] Selector defines ten ordered rules: finish active closeout → verify repo state → prefer audit/design before implementation → prefer ARC/docs/control-layer before risky trading edits → require Codex review before high-risk implementation → require explicit operator approval for the ten enumerated production-side actions → stop on ambiguous mode → stop on file-scope mismatch → stop on Codex FAIL/REQUIRED EDITS → stop on non-operator-signal-as-approval.
    - [x] Selector output (master order) populated: ARC-4 → ARC-5 (Claude/Codex/operator prompt templates) → ARC-6 (automation permission rules) → N-1 (repo confirmation) → N-2 (Migration 008 production-application planning) → N-3 (Migration 008 production application) → N-4 (D-5.12f live SELL_ALL design-only review).
    - [x] Hard rule: D-5.12f cannot start until ARC-4 / ARC-5 / ARC-6 are closed unless operator explicitly changes the master order.
    - [x] Registries updated: `orchestrator/PROTECTED-FILES.md` (3 places), `orchestrator/APPROVAL-GATES.md` (2 places), and `orchestrator/PHASE-MODES.md` (2 places) register `NEXT-ACTION-SELECTOR.md` as a safety-policy doc.
    - [x] ARC-4 status entries added to `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, `orchestrator/NEXT-ACTION.md`.
    ...
    - [ ] Codex docs-only review of the ARC-4 Path A docs-only diff across `NEXT-ACTION-SELECTOR.md`, `PHASE-MODES.md`, `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, `STATUS.md`, `CHECKLIST.md`, and `NEXT-ACTION.md`.
    - [ ] Operator approval to commit ARC-4 docs.
    - [ ] Commit exactly those seven ARC-4 Path A docs after Codex PASS + operator approval.
