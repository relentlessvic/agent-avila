...
**Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 in progress (Phase ARC-4 — mode: DOCS-ONLY).** ARC-4 (Next-Action Selector) is now active and docs-only: new `orchestrator/NEXT-ACTION-SELECTOR.md` drafted as a safety-policy doc enforcing strict ordering of future Agent Avila phases. Selector defines ten ordered rules (finish active closeout → verify repo state → prefer audit/design before implementation → prefer ARC/docs/control-layer before risky trading edits → require Codex review before high-risk implementation → require explicit operator approval for the ten enumerated production-side actions → stop on ambiguous mode → stop on file-scope mismatch → stop on Codex FAIL/REQUIRED EDITS → stop on non-operator-signal-as-approval). Selector output (master order) populated: ARC-4 → ARC-5 → ARC-6 → N-1 (repo confirmation) → N-2 (Migration 008 production-application planning) → N-3 (Migration 008 production application after Codex PASS + operator approval) → N-4 (D-5.12f live SELL_ALL design-only review). Hard rule: D-5.12f cannot start until ARC-4/ARC-5/ARC-6 are closed unless the operator explicitly changes the master order. Registries updated: `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, and `orchestrator/PHASE-MODES.md` register `NEXT-ACTION-SELECTOR.md` as a safety-policy doc. `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md` carry ARC-4 draft status entries. **No code, no commits, no migrations, no deploys, no edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` during ARC-4.** Pending Codex docs-only review of `NEXT-ACTION-SELECTOR.md` and the registry / status-doc edits (it is a safety-policy doc); pending explicit operator approval before any ARC-4 commit.
...
> **ARC-4 Codex docs-only review, then explicit operator approval before any ARC-4 commit.**

ARC-4 is in progress and not committed. The current Path A draft touches exactly `orchestrator/NEXT-ACTION-SELECTOR.md` (new safety-policy doc), `orchestrator/PHASE-MODES.md`, `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`. Next sequence: Codex docs-only review of the ARC-4 Path A diff (NEXT-ACTION-SELECTOR.md is a safety-policy doc, so per `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, and `PHASE-MODES.md` it requires Codex docs-only review and explicit operator approval before commit); if Codex returns PASS, ask the operator for explicit approval to commit exactly those seven paths; only after that approval, stage by name and commit. After ARC-4 closes, the selector advances to ARC-5 → ARC-6 → N-1 → N-2 → N-3 → N-4 per `orchestrator/NEXT-ACTION-SELECTOR.md`. The two pre-existing alternative candidates remain (operator may explicitly change the master order):
