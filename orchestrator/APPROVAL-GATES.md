...
| 10 | Any Ruflo install or automation upgrade (adding a new automation layer, widening an existing one, enabling new triggers, scheduling new agents). Includes Relay install — see `orchestrator/COMM-HUB-HERMES-RULES.md` (SAFE-class; filename retains `HERMES` literal pending Phase B) for the canonical Relay spec, anti-execution boundaries, approval discipline, and staged activation path. | Explicit operator approval | Docs-only or design review of the automation-upgrade plan |
...
- Approval-gate, phase-mode, next-action-selector, role-hierarchy, automation-permissions, or handoff-rules changes (Codex docs-only review of any change to this file, `PROTECTED-FILES.md`, `AUTOPILOT-RULES.md`, `BLUEPRINT.md`, `PHASE-MODES.md`, `NEXT-ACTION-SELECTOR.md`, `ROLE-HIERARCHY.md`, `AUTOMATION-PERMISSIONS.md`, or `HANDOFF-RULES.md`)
