...
All N-2x phases through N-2y are CLOSED (or, for N-2r, design-only PASS); N-2z is IN PROGRESS (DOCS-ONLY). The canonical detailed change history lives in `orchestrator/handoffs/N-2-MIGRATION-008-PRODUCTION-PLAN.md` §14; commit truth lives in `git log`.
...
| N-2z runbook clarification: §4(x)(a)/(b) Evidence-record form codification per Codex Pattern B | IN PROGRESS — DOCS-ONLY |
...
- [x] All five prior Victor production-action approvals (`9ae139d`, `3138e7f`, `e7bce599df678222ab0a58f0a4f3cc9f562f509a` Attempt 3, `e7bce599df678222ab0a58f0a4f3cc9f562f509a` Attempt 4, `726b2e33988b4ea4f31f94a3361144d3ce8e8fac` Attempt 5) remain CONSUMED and cannot be reused for N-3.
...
- [ ] Post-N-2z: operator runs Check D re-verification at the post-N-2z deployed HEAD via `railway ssh` (per the §4(x)(b) freshness rule) and attests the result in the next Codex N-3 preflight packet (per the N-2z-codified §4(x) Evidence-record form rule — operator-attested evidence in the preflight packet is canonical; HEAD-specific on-disk status records are not required for preflight PASS); a fresh Codex N-3 preflight at the post-N-2z committed HEAD; a fresh Victor in-session production-action approval naming that exact full HEAD are all required before Attempt 6.
...
- **Approved next-step sequence (after N-2z commit):** Codex re-review PASS on the N-2z four-file docs diff → operator approves N-2z commit → commit lands at a new HEAD → push to GitHub `origin/main` (GitHub-push-tracked deploy method per N-2t) → allow Railway auto-deploy (or scoped manual deploy approval) for the GitHub-tracked deploy → operator runs Check D re-verification at the post-N-2z deployed HEAD via `railway ssh` in-container `node --version` byte-for-byte against `.nvmrc` (per the N-2u-codified §4(x)(b) freshness rule) → operator includes operator-attested Check D PASS + Railway deploy-identity evidence in the next Codex N-3 preflight packet (per the N-2z-codified §4(x) Evidence-record form rule — packet attestation is canonical; HEAD-specific on-disk status records are NOT required for preflight PASS) → fresh Codex N-3 preflight at the post-N-2z committed HEAD → fresh Victor in-session production-action approval naming the post-N-2z HEAD → Attempt 6 from a fresh `railway ssh` session per the N-2m same-session rule, with corrected operator-side preflight tooling discipline per the N-2x §4(vii) rule (checker scripts under `/app`, not `/tmp`; host-side commands not pasted into the railway ssh container shell) AND the N-2y §4(vii) rule (CommonJS `require(...)` scripts MUST use `.cjs` extension under projects with `"type": "module"`, OR use ESM `import` syntax).
