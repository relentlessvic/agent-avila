...
**Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 in progress (Phase ARC-5 — mode: DOCS-ONLY).** ARC-5 (Role Hierarchy + Prompt Templates) is now active and docs-only: new `orchestrator/ROLE-HIERARCHY.md` drafted as a safety-policy / control doc establishing the formal Agent Avila role hierarchy and reusable prompt templates. Roles defined: Victor (CEO / Final Authority — sole approver of protected actions), ChatGPT (VP of Strategy & Orchestration — recommends, sequences, drafts prompts; cannot approve), Gemini (Director of Architecture / UX — advisory review; cannot approve), Claude (Lead Engineer / Builder — scoped execution; cannot self-approve), Codex (Chief Risk & Safety Officer — can block, but Codex PASS is never operator approval), and future Ruflo / automation (governance role only; cannot self-approve, cannot widen authority, cannot become a trading actor). Critical separation rule: the trading runtime (`bot.js`, live `dashboard.js` handlers, `db.js`, Strategy V2, Kraken adapter) is separate from the 3-brains governance system; brains govern changes to the runtime, not the runtime itself. Per-role responsibilities, authority boundaries, recommend / cannot-approve, escalation rules, and reusable prompt templates populated. Registries updated: `orchestrator/PROTECTED-FILES.md` (3 places), `orchestrator/APPROVAL-GATES.md` (2 places), and `orchestrator/PHASE-MODES.md` (2 places) register `ROLE-HIERARCHY.md` as a safety-policy doc. `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md` carry ARC-5 draft status entries. **No code, no commits, no migrations, no deploys, no edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` during ARC-5.** Pending Codex docs-only review of `ROLE-HIERARCHY.md` and the registry / status-doc edits (it is a safety-policy / control doc); pending explicit operator approval before any ARC-5 commit.
...
> **ARC-5 Codex docs-only review, then explicit operator approval before any ARC-5 commit.**

ARC-5 is in progress and not committed. The current draft touches exactly `orchestrator/ROLE-HIERARCHY.md` (new safety-policy / control doc), `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, `orchestrator/PHASE-MODES.md`, `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`. Next sequence: Codex docs-only review of the ARC-5 diff (ROLE-HIERARCHY.md is a safety-policy doc, so per `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, and `PHASE-MODES.md` it requires Codex docs-only review and explicit operator approval before commit); if Codex returns PASS, ask the operator for explicit approval to commit exactly those seven paths; only after that approval, stage by name and commit. After ARC-5 closes, the selector advances to ARC-6 → N-1 → N-2 → N-3 → N-4 per `orchestrator/NEXT-ACTION-SELECTOR.md`. The two pre-existing alternative candidates remain (operator may explicitly change the master order):
