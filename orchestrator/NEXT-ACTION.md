...
> **N-2l Codex docs-only review of the Node exact-pin whitespace-normalization (parsing-hygiene) tightening runbook update, then explicit operator approval before any N-2l commit; N-3 remains hard-blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2l, fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven pre-flight checks (i)–(xi) PASS, target confirmation without exposing secrets).**
...
- Before applying Migration 008: follow the current N-2l runbook gate — fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven §4 pre-flight checks (i)–(xi) PASS at execution time (including the §4(x) three-component runtime-source identity check — source revision + exact Node `major.minor.patch` pin (highest-priority present source canonical; HALT on disagreement among multiple exact-pin sources; non-exact `"engines.node"` ranges do NOT satisfy (b); whitespace / newline / leading-`v` normalization applied before exactness testing) + dependency / build identity), target confirmation without exposing secrets, then the full §6 verification after runner exit.
...
For **Migration 008 production-application planning** (when authorized): the operator-driven sequence is — (1) satisfy the current N-2l runbook gate, including fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven §4 pre-flight checks (i)–(xi) PASS at execution time (including the §4(x) three-component runtime-source identity check with an exact Node `major.minor.patch` pin — highest-priority present source canonical; HALT on disagreement among multiple exact-pin sources; whitespace / newline normalization applied before exactness testing), and target confirmation without exposing secrets; (2) apply Migration 008 only under that explicit production-action approval; (3) complete the full §6 post-application verification; (4) document the application timestamp in `STATUS.md`. After application, D-5.12d/e production deployment is unblocked (still requires its own explicit operator authorization).
