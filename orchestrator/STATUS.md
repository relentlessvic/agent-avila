...
**Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-2c closed (`9ae139d`), N-3 attempt 2026-05-03 HALTED (runner exit 1; `getaddrinfo ENOTFOUND` for Railway-internal Postgres hostname; no SQL executed; production-DB unchanged; Migration 008 NOT applied; prior Victor approval consumed), N-2d in progress (Phase N-2d — mode: DOCS-ONLY).** N-2d is a connectivity-preflight runbook update phase: the N-3 attempt failed before any SQL was executed because the runner was invoked from a non-Railway-attached execution context where the internal Postgres hostname does not resolve. N-2d adds (a) §1.1 N-3 attempt history; (b) §3 Railway-attached execution-context requirement; (c) §4 pre-flight check (vii) connectivity preflight; (d) §7 emergency stop conditions for unreachable-target and connection-failure, plus an explicit "do not retry automatically" rule; (e) two new risk-table rows (P0 local context cannot resolve Railway-internal hostname; P1 retrying with a consumed approval). The runbook remains fully prose-only and HANDOFF-RULES.md compliant — no literal Railway runner commands, no `DATABASE_URL` examples, no hostname/port/URL values added. **Migration 008 remains NOT applied to production.** **No production-DB state changed during the N-3 attempt — the runner exited at the connection step before any transaction was opened. No rollback is needed and none is authorized.** N-3 remains hard-blocked pending: (1) Codex docs-only PASS on this N-2d update; (2) fresh Codex N-3 preflight PASS on the updated runbook; (3) fresh Victor explicit in-session production-action approval naming the exact current HEAD; (4) all seven pre-flight checks (i)–(vii) PASS at execution time; (5) target Railway service / production database confirmation without exposing secrets. **No code, no migrations applied, no deploys, no Railway commands invoked by Claude, no production-DB queries by Claude, no env / secret access, no `MANUAL_LIVE_ARMED` change, no live Kraken actions, no edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` during N-2d.** Pending Codex docs-only review of this runbook update + status-doc edits, then explicit operator approval before any N-2d commit. N-2c is a HEAD-preflight correction phase: Codex N-3 preflight review caught that the committed N-2b runbook still pinned pre-flight HEAD to `8266db2…` (the pre-N-2b commit), while the runbook is now committed at `e6c9189`. Naively updating the pinned hash to `e6c9189…` would produce the same staleness the moment N-2c itself commits (self-staling commit loop). N-2c instead replaces the in-runbook hardcoded HEAD pin with an approval-time exact-HEAD-naming rule: pre-flight check (i) now requires HEAD to equal the exact hash named in Victor's N-3 approval; §5 adds an "exact-HEAD-naming requirement" that binds Victor's approval to a specific HEAD and expires the approval if HEAD changes (new commit, rebase, fast-forward pull) before execution; the risk table is reformulated to require approval-time HEAD-naming and HALT-on-HEAD-change. N-3 remains hard-blocked pending fresh Codex N-3 preflight PASS on the corrected runbook + Victor explicit in-session production-action approval naming the exact current HEAD + execution-time pre-flight pass + target Railway service / production `DATABASE_URL` confirmation without exposing secrets. **No code, no migrations, no deploys, no Railway commands, no production-DB queries, no env / secret access, no `MANUAL_LIVE_ARMED` change, no live Kraken actions, no edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` during N-2c.** Pending Codex docs-only review and explicit operator approval before any N-2c commit. N-2b is closed at `e6c9189`; N-2c is the only active docs-only correction phase. N-3 remains hard-blocked behind fresh Codex N-3 preflight PASS on the corrected runbook + Victor explicit in-session production-action approval naming the exact current HEAD + execution-time pre-flight pass + target Railway service / production `DATABASE_URL` confirmation without exposing secrets.
...
> **N-2d Codex docs-only review of the connectivity-preflight runbook update; then explicit operator approval before any N-2d commit; N-3 remains hard-blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2d, fresh Codex N-3 preflight PASS on the updated runbook, fresh Victor approval naming exact current HEAD, all seven pre-flight checks PASS, target confirmation without exposing secrets).**

N-2c closed (`9ae139d`). On 2026-05-03, a single N-3 attempt was made and HALTED before any SQL executed: the runner exited 1 with `getaddrinfo ENOTFOUND` for the Railway-internal Postgres hostname, because the execution context (operator's local machine) cannot resolve Railway's internal-network DNS. No `applying` log line appeared, no `BEGIN` was issued, no SQL was executed. Production-DB state is unchanged; Migration 008 is still NOT applied; no partial state; no rollback needed and none authorized. The prior Victor approval was consumed by the failed attempt. **N-2d (DOCS-ONLY)** updates the runbook to add: (a) an §1.1 N-3 attempt history; (b) a §3 Railway-attached execution-context requirement; (c) a new pre-flight check (vii) connectivity preflight; (d) §7 HALT-on-unreachable-target and HALT-on-connection-failure stop conditions plus a do-not-retry-automatically rule; (e) two new risk-table rows (P0 hostname-resolution; P1 consumed-approval). The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` examples, no hostname/port/URL values added. The current draft touches exactly the runbook plus minimal status entries in this file, `CHECKLIST.md`, and `NEXT-ACTION.md`. No safety-policy doc is changed in N-2d. Next sequence: Codex docs-only review of the N-2d diff + the three status-doc edits; on Codex PASS, ask the operator for explicit approval to commit; only after that approval, stage by name and commit. After N-2d commit, **N-3 remains hard-blocked** until all five gate conditions are simultaneously satisfied with a fresh Codex N-3 preflight PASS on the now-updated runbook and a fresh Victor in-session production-action approval naming the exact current HEAD at retry time. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2d commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-3 are all handled per the master order. The two pre-existing alternative candidates remain (operator may explicitly change the master order):
