...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 in progress (Phase ARC-5 — mode: DOCS-ONLY).**
  ...
  - [~] **ARC-5 — Role Hierarchy + Prompt Templates (Phase ARC-5 — mode: DOCS-ONLY) — IN PROGRESS.**
    - [x] New `orchestrator/ROLE-HIERARCHY.md` drafted as a safety-policy / control doc establishing the formal Agent Avila role hierarchy.
    - [x] Roles defined: Victor (CEO / Final Authority — sole approver of protected actions), ChatGPT (VP of Strategy & Orchestration — recommends, sequences, drafts prompts; cannot approve), Gemini (Director of Architecture / UX — advisory review; cannot approve), Claude (Lead Engineer / Builder — scoped execution; cannot self-approve), Codex (Chief Risk & Safety Officer — can block, but Codex PASS is never operator approval), and future Ruflo / automation (governance role only; cannot self-approve, cannot widen authority, cannot become a trading actor).
    - [x] Critical separation rule documented: the trading runtime (`bot.js`, live `dashboard.js` handlers, `db.js`, Strategy V2, Kraken adapter) is separate from the 3-brains governance system; brains govern changes to the runtime, not the runtime itself.
    - [x] Per-role responsibilities, authority boundaries, recommend / cannot-approve, and escalation rules populated.
    - [x] Reusable prompt templates for ChatGPT (VP), Gemini (Director), Claude (Builder), and Codex (CRSO) included.
    - [x] Cross-references to all four pre-existing safety-policy docs (PROTECTED-FILES.md, APPROVAL-GATES.md, PHASE-MODES.md, NEXT-ACTION-SELECTOR.md) plus AUTOPILOT-RULES.md, BLUEPRINT.md, STATUS.md, NEXT-ACTION.md, CLAUDE.md.
    - [x] Registries updated: `orchestrator/PROTECTED-FILES.md` (3 places), `orchestrator/APPROVAL-GATES.md` (2 places), and `orchestrator/PHASE-MODES.md` (2 places) register `ROLE-HIERARCHY.md` as a safety-policy doc.
    - [x] ARC-5 status entries added to `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, `orchestrator/NEXT-ACTION.md`.
    ...
    - [ ] Codex docs-only review of the ARC-5 docs-only diff across `ROLE-HIERARCHY.md`, `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, `PHASE-MODES.md`, `STATUS.md`, `CHECKLIST.md`, and `NEXT-ACTION.md`.
    - [ ] Operator approval to commit ARC-5 docs.
    - [ ] Commit exactly those seven ARC-5 docs after Codex PASS + operator approval.
