...
**Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 in progress (Phase ARC-6 — mode: DOCS-ONLY).** ARC-6 (Automation Permission Rules) is now active and docs-only: new `orchestrator/AUTOMATION-PERMISSIONS.md` drafted as a safety-policy / control doc establishing the formal Agent Avila automation-permission rules. Three-tier model defined: GREEN (safe automatic — read-only inspection, file reads, search, drafting docs/design reports/plans, Codex review prep, syntax checks) — provided the active phase mode allows it; YELLOW (requires Codex review — drafting safety-policy doc edits, RESTRICTED file diffs, HIGH-RISK design reports, PRODUCTION ACTION plans, Claude Code permission changes — automation may draft but cannot commit/apply/deploy without Codex PASS); RED (requires explicit Victor / CEO approval and cannot be done automatically — bot.js / db.js / live dashboard handler edits, migration file edits, migration application, Railway deploys, live Kraken actions, position.json writes, real-money behavior, MANUAL_LIVE_ARMED actions, env/secret changes, destructive git, automation widening, Ruflo/Hermes install or upgrade, phase-mode promotion, master-order changes, safety-policy doc commits). Blocked-commands list enumerates destructive git, production deploy, migration runner, production DB writes, position.json writes, runtime-file edits, env/secret writes, live Kraken triggers, automation widening. Stop conditions cover RED-without-approval, blocked-command attempts, blocked-file modifications, Codex non-PASS treated as approval, non-operator-signal-as-approval, ambiguous mode, scoped-lift widening, unauthorized live Kraken / deploy / migration triggers, scheduled triggers firing YELLOW/RED actions, automation install/upgrade/expand attempts. Future-automation rules (Ruflo, Hermes, successors) explicitly inherit governance-only role; cannot become trading actors; cannot self-approve; cannot widen authority. Trading-runtime separation rule reinforced. Registries updated: `orchestrator/PROTECTED-FILES.md` (3 places), `orchestrator/APPROVAL-GATES.md` (2 places), and `orchestrator/PHASE-MODES.md` (2 places) register `AUTOMATION-PERMISSIONS.md` as a safety-policy doc. `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md` carry ARC-6 draft status entries. **No code, no commits, no migrations, no deploys, no edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` during ARC-6.** Pending Codex docs-only review of `AUTOMATION-PERMISSIONS.md` and the registry / status-doc edits (it is a safety-policy / control doc); pending explicit operator approval before any ARC-6 commit.
...
> **ARC-6 Codex docs-only review, then explicit operator approval before any ARC-6 commit.**

ARC-6 is in progress and not committed. The current draft touches exactly `orchestrator/AUTOMATION-PERMISSIONS.md` (new safety-policy / control doc), `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, `orchestrator/PHASE-MODES.md`, `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`. Next sequence: Codex docs-only review of the ARC-6 diff (AUTOMATION-PERMISSIONS.md is a safety-policy doc, so per `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, and `PHASE-MODES.md` it requires Codex docs-only review and explicit operator approval before commit); if Codex returns PASS, ask the operator for explicit approval to commit exactly those seven paths; only after that approval, stage by name and commit. After ARC-6 closes, the selector advances to N-1 → N-2 → N-3 → N-4 per `orchestrator/NEXT-ACTION-SELECTOR.md`. The two pre-existing alternative candidates remain (operator may explicitly change the master order):
