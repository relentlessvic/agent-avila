...
- **Original Railway service name `agent-avila-hermes`** — preserved as historical fact. Victor manually renamed the Railway service from `agent-avila-hermes` to `agent-avila-relay` on 2026-05-08 during the operator-led RAILWAY-SERVICE-RENAME-HERMES-TO-RELAY-MANUAL Phase 3 (no commit; docs cleanup deferred to Phase 4). The current Railway service display name is `agent-avila-relay`. Both literals (`agent-avila-hermes` historical / `agent-avila-relay` current) are preserved verbatim in historical / SHA-anchored references.
