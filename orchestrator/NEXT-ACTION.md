...
**Hands-Free ARC track — ARC-1 closed (`95157ae`); ARC-2 in progress (docs-only).** ARC-0 (read-only current-state verification) PASSed on 2026-05-02. ARC-1 (Orchestrator Protection Matrix) closed with Codex final PASS and operator approval; committed `95157ae` (4 files, +173 / 0). ARC-2 (Approval Gate Matrix) is now active and docs-only: substantial expansion of `orchestrator/APPROVAL-GATES.md` adds the numbered 16-gate matrix, "what does NOT need operator approval" list, "Codex review BEFORE operator approval" requirements, structured "what an approval must include" form, "what is NOT operator approval" non-equivalence list, "production actions require separate approval" separation rule, refreshed stop conditions, and cross-references to `PROTECTED-FILES.md`. Original five sections preserved. `orchestrator/PROTECTED-FILES.md` left unchanged (referenced as complementary; not rewritten). Minimal ARC-2 status entries added to `STATUS.md`, `CHECKLIST.md`, and this file. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migrations, no deploys, no live Kraken actions. No commit yet.** Pending Codex docs-only review and operator approval.

The next allowed action after this ARC-2 doc draft is: (a) Codex docs-only review of the expanded `orchestrator/APPROVAL-GATES.md` and the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), operator approval to commit; (c) commit `orchestrator/*` only — never stage or modify any HARD BLOCK file. After ARC-2 commit, the operator-chosen next phase below remains the operative path.
