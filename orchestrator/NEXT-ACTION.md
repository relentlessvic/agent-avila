...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 in progress (Phase ARC-6 — mode: DOCS-ONLY).** ARC-0 (read-only current-state verification) PASSed on 2026-05-02. ARC-1 through ARC-5 closed in sequence. ARC-6 (Automation Permission Rules) is now active and docs-only: new `orchestrator/AUTOMATION-PERMISSIONS.md` is a safety-policy / control doc establishing the formal automation-permission rules for Claude / Codex / ChatGPT / Gemini / Ruflo / Hermes / any future automation. Three-tier model: GREEN (safe automatic when phase mode allows), YELLOW (requires Codex review), RED (requires explicit Victor / CEO approval and cannot be done automatically). Blocked-commands list, blocked-files list, stop conditions, and future-automation governance-only inheritance documented. Trading-runtime separation reinforced. Registries updated: `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, and `orchestrator/PHASE-MODES.md` register `AUTOMATION-PERMISSIONS.md` as a safety-policy doc. ARC-6 status entries added to `STATUS.md`, `CHECKLIST.md`, and this file. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migrations, no deploys, no live Kraken actions. No commit yet.** Pending Codex docs-only review and explicit operator approval (AUTOMATION-PERMISSIONS.md is a safety-policy doc).

The next allowed action after this ARC-6 doc draft is: (a) Codex docs-only review of `orchestrator/AUTOMATION-PERMISSIONS.md`, `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, `orchestrator/PHASE-MODES.md`, `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit; (c) commit exactly those seven paths only — never stage or modify any HARD BLOCK file. After ARC-6 commit, the selector advances to N-1 → N-2 → N-3 → N-4 per `orchestrator/NEXT-ACTION-SELECTOR.md`.
