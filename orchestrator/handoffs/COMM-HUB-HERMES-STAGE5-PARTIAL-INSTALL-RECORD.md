...
Last updated: 2026-05-06 (COMM-HUB-HERMES-INSTALL-CLOSEOUT — DOCS-ONLY)
Canonical references:
- `orchestrator/COMM-HUB-HERMES-RULES.md` — canonical Hermes specification (SAFE-class)
- `orchestrator/handoffs/COMM-HUB-INSTALL-HERMES-CHECKLIST.md` — Hermes Stage 5 install checklist (the 21-step canonical sequence)
...
- A Hermes runtime image / process binary that does not exist today.
...
| 2 | Create application "Agent Avila Hermes" | ✅ Application created; landing page loaded successfully |
...
| 9 | Invite bot to `Agent Avila Hub` ONLY | ✅ Install URL opened in primary-account browser; selected server `Agent Avila Hub`; verified scope=bot and permissions=Send Messages + View Channels on the authorize page; clicked Authorize; CAPTCHA cleared; bot `Agent Avila Hermes` now visible in `Agent Avila Hub` member list; no messages posted in any channel; bot not invited to any other server |
...
- ✅ Bot has no posting capability without a running Hermes process; no process is running.
...
|---|---|---|
| 14 | Provision the Hermes container/image on the chosen host (Railway service `agent-avila-hermes`); enforce egress allowlist; configure non-root non-privileged user; verify no git checkout / no DB credentials / no Kraken credentials / no Railway credential leakage / no `MANUAL_LIVE_ARMED` env var; configure logging endpoint env var | Hermes runtime code does not exist; no image to deploy |
| 15 | Provision Hermes-private append-only publish log on host | Coupled to runtime; deferred with Step 14 |
...
| 18 | Smoke-test connectivity (boot, Discord gateway IDENTIFY+READY, channel-list inspection of 3 allowed + verification of 4 forbidden NOT visible); NO message published | Requires running Hermes process; deferred |
| 19 | Stop the Hermes process at end of install | No process to stop; vacuously satisfied (state at end of Steps 1–13 has no running Hermes process) |
...
## §6 — Effective state of Hermes (post-closeout, no runtime running)

After Stage 6 closeout commits and pushes, Hermes is in a "shelved" state:

- **Discord-side:** the bot `Agent Avila Hermes` is a passive member of `Agent Avila Hub` with the `System-Writer` role + canonical channel overrides (allow on 3 + explicit deny on 4). The bot **cannot post anywhere** because no Hermes process is running. The bot exists only as a Discord-side member object; without a running runtime authenticated with the bot token, no Discord API call is made.
...
- **Trading-runtime-side:** completely separate. Hermes has no link to `agent-avila-dashboard`, the production DB, Kraken, env, `MANUAL_LIVE_ARMED`, or `position.json`.
- **Process-state:** no Hermes process exists anywhere. The Stage 5 approval scope did not authorize the operator to write or build a Hermes runtime; no runtime authoring track has opened. Hermes is, in effect, a Discord-side member with a stored token but no actual program using it.
- **Posting capability:** zero. Even if the operator started a third-party Discord-bot framework with the stored token outside the orchestrator framework, that would be (a) unauthorized per `orchestrator/COMM-HUB-HERMES-RULES.md`'s anti-execution boundaries, (b) not in scope of the consumed Stage 5 approval, and (c) detectable through log evidence and channel-content inspection. The orchestrator framework provides no path to "accidentally" post via Hermes from the shelved state.
...
- **Trading authority:** zero (forever forbidden per Hermes spec).
- **Approval authority:** zero (forever forbidden per Hermes spec).
...
If the operator decides to abandon the Hermes track entirely or revert Hermes to fully DORMANT state, three reversible steps:
...
After these 3 steps, Hermes is fully DORMANT in the same state as before Stage 5 began (zero members, zero permissions on `System-Writer` role still set per Step 11 — but with zero members; no Railway service; no token anywhere).

**Optional 4th step** (only if operator wants to remove the Hermes Discord application entirely): Discord developer portal → Application → Delete App → confirm. This deletes the application identity itself; future Hermes installs would need a fresh application registration.
...
### 8.1 Hermes runtime authoring track

Before Steps 14–21 can resume, a Hermes runtime image / process binary must exist. Authoring this runtime is its own substantive implementation track, distinct from the DOCS-ONLY / DESIGN-ONLY phases that produced the Hermes spec, install checklist, dry-run design, and preconditions doc. Suggested phase sequence (each requires its own separate operator approval):

- **`COMM-HUB-HERMES-RUNTIME-DESIGN`** (DESIGN-ONLY conversation) — design the Hermes process architecture: language / framework choice (Discord client library compatibility), idempotency-store data model, source-of-truth message store schema, halt-on-anomaly state machine, container image base, env variable handling, logging output format, signal-handling for clean exit, single-instance discipline mechanism, etc. Codex design review at end. Operator-directed manual; conversation-only; no commit.
...
After a Hermes runtime image exists and is operator-approved, Stage 5 install can resume:
...
- **`COMM-HUB-HERMES-INSTALL-RESUME`** (or named-equivalent; operator-chosen) — opens at a fresh HEAD, requires:
  ...
  - The operator deploys the Hermes runtime to the existing `agent-avila-hermes` Railway service shell + configures egress allowlist + smoke-tests connectivity (no publish) + stops the process + captures evidence + opens a Stage 6 closeout for the resumption.
...
- **Hermes effectively DORMANT** from a posting-capability perspective. The `System-Writer` role has minimum-active permissions per Step 11 (NOT zero permissions as canonical DORMANT — but the role has only `View Channels` + `Send Messages` and no Hermes process is running, so no actual posting occurs).
...
- Hermes install resumption (Steps 14–21).
- Hermes runtime authoring or implementation.
- Hermes runtime deployment to the `agent-avila-hermes` Railway service.
- Hermes process boot or smoke-test execution.
...
- Discord application registration beyond the existing `Agent Avila Hermes`.
...
- Granting Hermes `Read Message History` (forever forbidden by canonical Hermes spec).
- Granting Hermes any approval authority (forever forbidden).
- Granting Hermes any trading authority (forever forbidden).
...
- Modification of canonical Hermes spec, install checklist, dry-run design, preconditions doc, channel layout, or any safety-policy doc.

**Each of the deferred Steps 14–21, the runtime authoring track, and any later Hermes activation stage requires its own separately-approved phase with its own design / Codex review / explicit Victor approval cascade.**

---

## What this document is NOT

- **Not authorization to resume Stage 5 install (Steps 14–21).** Resumption is a separately operator-approved phase with a fresh Gate-10 approval and a Hermes runtime image that does not yet exist.
- **Not authorization to author or deploy a Hermes runtime.** Runtime authoring is a separate substantive implementation track with its own design / docs / implement / closeout phase sequence.
- **Not authorization to start the Hermes process.** The `agent-avila-hermes` Railway service exists with a token, but no runtime is deployed; the operator cannot start a non-existent runtime, and the closeout does not authorize starting one even if it did exist.
- **Not authorization to post any Discord message via Hermes.** Discord posting via Hermes is gated behind Stages 9 / 10a / 10b, each separately approved.
...
- **Not canonical over `orchestrator/COMM-HUB-HERMES-RULES.md`.** If this record diverges from the Hermes spec, the spec wins.
...
**This Stage 6 closeout phase (`COMM-HUB-HERMES-INSTALL-CLOSEOUT`) is DOCS-ONLY and records the partial Stage 5 install. Hermes is in a "shelved" state with no posting capability. Stage 5 Gate-10 install approval is CONSUMED. Steps 14–21 require a fresh separately-approved phase with a Hermes runtime that does not exist today.**
