...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 in progress (docs-only).**
  ...
  - [~] **ARC-3 — Phase Mode System (docs-only) — IN PROGRESS.**
    - [x] New `orchestrator/PHASE-MODES.md` drafted defining six modes (READ-ONLY AUDIT, DESIGN-ONLY, DOCS-ONLY, SAFE IMPLEMENTATION, HIGH-RISK IMPLEMENTATION, PRODUCTION ACTION) with allowed actions, blocked actions, required reviews, required approvals, and stop conditions per mode.
    - [x] Phase-labeling rule mandates that every future Agent Avila phase be labeled with exactly one mode before work begins (label format: `Phase <name> — mode: <MODE NAME>`).
    - [x] Ambiguous-mode rule: Claude must stop and ask the operator; default fallback is the higher-risk candidate mode.
    - [x] Automation/Ruflo rule: automation cannot upgrade phase mode and cannot self-approve.
    - [x] Path A follow-up applies Codex-required safety-policy cross-reference wording edits to `orchestrator/PROTECTED-FILES.md` and `orchestrator/APPROVAL-GATES.md`.
    - [x] ARC-3 status entries added to `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, `orchestrator/NEXT-ACTION.md`.
    ...
    - [ ] Codex docs-only review of the ARC-3 Path A docs-only diff across `PHASE-MODES.md`, `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, `STATUS.md`, `CHECKLIST.md`, and `NEXT-ACTION.md`.
    - [ ] Operator approval to commit ARC-3 docs.
    - [ ] Commit exactly those six ARC-3 docs after Codex PASS + operator approval.
