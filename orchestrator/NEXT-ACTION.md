...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-2c closed (`9ae139d`), N-3 attempt-1 2026-05-03 HALTED at HEAD `9ae139d` (DNS / execution-context failure; Migration 008 NOT applied; approval consumed), N-2d closed (`3732721`), N-2e closed (`afe94d1`), N-2f closed (`3af1e44`), N-2g closed (`926eb7f`), N-2h closed (`ea7774d`), N-2i closed (`5ee1dcb`), N-2j closed (`548383b`), N-2k closed (`b2d187d`), N-2l closed (`3138e7f`), **N-3 attempt-2 2026-05-03 HALTED at HEAD `3138e7f`**, N-2m closed (`6b9be1d`), N-2n closed (`8fc53b9`), **fresh Codex N-3 preflight on the N-2n-committed runbook at HEAD `8fc53b9` returned FAIL** with six required edits (GAP-A through GAP-G; GAP-F + GAP-H sufficient), N-2o in progress (Phase N-2o — mode: DOCS-ONLY).** N-2o is a deployed-runtime shell execution hardening phase. The fresh Codex N-3 preflight on the N-2n-committed runbook returned FAIL with six required edits: (GAP-A) explicit HALT branches when the Railway-attached deployed-runtime shell is unavailable, no running container exists on the deployed `agent-avila-dashboard` service, shell-access credential / key setup fails, or no usable interactive shell binary is present in the deployed container; (GAP-B) deployed-runtime working-directory verification before §4(vii) and runner invocation; (GAP-C) approved migration runner invocation form specified in prose only with `scripts/run-migrations.js` existence verification at the expected path inside the deployed container before invocation; HALT on missing script or unverified path; no literal production runner commands written into the runbook; (GAP-D) acknowledgment that `git` may be absent from the deployed NIXPACKS container with fallback to a non-secret Railway deploy-metadata surface (deploy-commit SHA in the Railway dashboard, deployment-log header, or approved read-only Railway API), full 40-character SHA byte-for-byte against Victor's approved HEAD; HALT if source identity cannot be proven without exposing secrets; (GAP-E) two-context SHA comparison rule when both in-container `git rev-parse HEAD` AND a non-secret Railway deploy-metadata surface are available; HALT on any disagreement; if `git` is unavailable, Railway deploy-metadata surface alone may serve as identity proof if it returns a full 40-character SHA matching Victor's approved HEAD; (GAP-G) replacement of stale §1 "§11 condition (1) updated to N-2m; §9 gate-separation chain extended through N-2m" wording with the N-2o-correct equivalent. **N-2o (DOCS-ONLY)** applies all six edits to the runbook §3, §4(vii), §4(x)(a), §1, §11, §9, and §14. **Both Victor approvals (`9ae139d`, `3138e7f`) remain CONSUMED.** §11 condition (1) updated to N-2o. §9 gate-separation chain extended through N-2o. No §4(i)–(vi) / §4(viii) / §4(ix) / §4(x)(b) / §4(x)(c) / §4(xi) / §5 / §6 / §15 substance changed; §3 + §4(vii) + §4(x)(a) are extended; §11 / §9 phase labels updated. Pre-flight remains at eleven checks (i)–(xi); no new check added. The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` values, no hostname / port / URL values added. **N-3 remains halted/blocked.** No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` / `package.json` / lockfiles. No retry. No rollback. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet. Pending Codex docs-only review and explicit operator approval before any N-2o commit. **N-2o commit-time approval does NOT authorize N-3.**

The next allowed action after this N-2o doc draft is: (a) Codex docs-only review of the runbook diff + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit N-2o; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2o commit, **N-3 remains halted/blocked** behind the five-condition gate, with both prior Victor approvals (`9ae139d` and `3138e7f`) already consumed. Any future N-3 attempt requires fresh Codex N-3 preflight PASS at the new HEAD, fresh Victor approval naming the new HEAD, AND operator-side execution against the approved primary surface — a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container (per N-2n's §3 + §4(vii) gating, with the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with script-existence verification, the §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and the two-context SHA comparison rule), NOT a local `railway run` / `railway shell` session. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2o commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h / N-2i / N-2j / N-2k / N-2l / N-2m / N-2n / N-2o / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
...
> **N-2o Codex docs-only review of the deployed-runtime shell execution hardening runbook update, then explicit operator approval before any N-2o commit; N-3 remains halted/blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2o, fresh Codex N-3 preflight PASS, fresh Victor approval naming a NEW HEAD — both `9ae139d` and `3138e7f` approvals are consumed, all eleven pre-flight checks (i)–(xi) PASS at execution time from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container per the N-2n §3 + §4(vii) gating with the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with script-existence verification, §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and two-context SHA comparison rule, target confirmation without exposing secrets).**
...
- Before applying Migration 008: follow the current N-2o runbook gate — fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD (note: both prior approvals at `9ae139d` and `3138e7f` are consumed), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time **from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container** (the approved primary execution surface per N-2n; local Railway CLI run-mode / shell-mode are explicitly REJECTED for internal-hostname `DATABASE_URL`) — including the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with `scripts/run-migrations.js` existence verification at the expected path inside the deployed container before invocation, the §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and the two-context SHA comparison rule — target confirmation without exposing secrets, then the full §6 verification after runner exit.
...
For **Migration 008 production-application planning** (when authorized): the operator-driven sequence is — (1) satisfy the current N-2o runbook gate, including fresh Codex N-3 preflight PASS, fresh Victor approval naming a new HEAD (both prior approvals at `9ae139d` and `3138e7f` are consumed by HALTED attempts), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container (the approved primary execution surface per N-2n; local Railway CLI run-mode / shell-mode are explicitly REJECTED), with the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with script-existence verification, the §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and the two-context SHA comparison rule, and target confirmation without exposing secrets; (2) apply Migration 008 only under that explicit production-action approval; (3) complete the full §6 post-application verification; (4) document the application timestamp in `STATUS.md`. After application, D-5.12d/e production deployment is unblocked (still requires its own explicit operator authorization).
