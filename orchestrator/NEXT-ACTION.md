...
`N-2b e6c9189` -> `N-2c 9ae139d` -> `N-2d 3732721` -> `N-2e afe94d1` -> `N-2f 3af1e44` -> `N-2g 926eb7f` -> `N-2h ea7774d` -> `N-2i 5ee1dcb` -> `N-2j 548383b` -> `N-2k b2d187d` -> `N-2l 3138e7f` -> `N-2m 6b9be1d` -> `N-2n 8fc53b9` -> `N-2o f925ac5` -> `N-2p ddca950` -> `N-2q 29ac7d7` -> `N-2r (design-only PASS, no commit)` -> `N-2s 6c3a1e5 (superseded by GitHub-tracked deploy at 49650f0…)` -> `N-2t 49650f077509d83dcbf3e9771dc9ca30f351e55b (CLOSED — deploy-method source-identity gating)` -> `N-2u 0a6974884849dd1eb6bdf4f88cb5d41085044612 (CLOSED — Codex Q10 freshness-invalidation runbook fix)` -> `N-2v 3c4777224dafd6c09c47004631c7c8ba538a2b3b (CLOSED — post-N-2u stale-tail repair + Check D re-verification record)` -> `N-2w e7bce599df678222ab0a58f0a4f3cc9f562f509a (CLOSED — single-line stale-tail repair on NEXT-ACTION.md:34)` -> `N-2x (current — DOCS-ONLY operator-side preflight checker module-resolution rule + host/container command-boundary rule + N-3 attempt history back-fill (Attempts 3, 4) + §14 back-fill (N-2v, N-2w))`.

**N-2x (current) applies Codex's exact Q12 Path B required wording from the post-attempt-4 fresh N-3 preflight at HEAD `e7bce599df678222ab0a58f0a4f3cc9f562f509a` to runbook §4(vii)** (operator-side preflight checker scripts MUST run from `/app` or with `NODE_PATH=/app/node_modules`; `/tmp` REJECTED; HALT before SQL on FAIL; host-side commands explicitly marked and not pasted into railway ssh container shell). N-2x also back-fills runbook §1.1 with Attempt 3 (pre-SQL preflight tooling halt at `/tmp` module resolution; consumed approval; runner not invoked) and Attempt 4 (recurrence + host-vs-container command confusion; consumed approval; runner not invoked), and back-fills runbook §14 with N-2v + N-2w + N-2x history. N-2v (closed at HEAD `3c4777224dafd6c09c47004631c7c8ba538a2b3b`) was the post-N-2u stale-tail repair + Check D re-verification record. N-2w (closed at HEAD `e7bce599df678222ab0a58f0a4f3cc9f562f509a`) was the single-line stale-tail repair on NEXT-ACTION.md:34. N-2u (closed at HEAD `0a6974884849dd1eb6bdf4f88cb5d41085044612`) added Codex's exact Q10 freshness-invalidation paragraph to runbook §4(x)(b).

The next allowed action sequence after N-2x commits: (1) operator approves the N-2x DOCS-ONLY commit (runbook + STATUS / CHECKLIST / NEXT-ACTION); (2) N-2x commit lands at a new HEAD per `git rev-parse HEAD`; (3) push to GitHub `origin/main` (separately approved); (4) Railway auto-deploy redeploys `agent-avila-dashboard` via the GitHub-push-tracked deploy method, recording the post-N-2x full 40-character commit SHA on Railway's non-secret deploy-metadata surface; (5) operator re-verifies Check D at the post-N-2x deployed HEAD (per `git rev-parse HEAD` after commit) via `railway ssh` in-container `node --version` byte-for-byte against `.nvmrc` (the §4(x)(b) freshness rule fires on every redeploy); (6) fresh Codex N-3 preflight at the post-N-2x committed HEAD; (7) fresh Victor in-session production-action approval naming the exact full post-N-2x 40-character HEAD; (8) Attempt 5 from a fresh `railway ssh` session per the N-2m same-session rule, with corrected operator-side preflight tooling discipline per the N-2x §4(vii) rule (checker scripts under `/app`, not `/tmp`; host-side commands not pasted into the railway ssh container shell).

## Required Gate State

**N-2s commit-time approval authorized only committing `.nvmrc`, `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`; it did NOT authorize a deploy and did NOT authorize N-3.** **N-2t commit-time approval (consumed by commit `49650f0…`) authorized only committing the runbook + three status docs; it did NOT authorize a deploy and did NOT authorize N-3.** **N-2u commit-time approval (consumed by commit `0a6974884849dd1eb6bdf4f88cb5d41085044612`) authorized only committing the runbook + three status docs (Codex Q10 freshness-invalidation edit); it did NOT authorize a deploy or N-3.** **N-2v commit-time approval (consumed by commit `3c4777224dafd6c09c47004631c7c8ba538a2b3b`) authorized only committing the three status docs; it did NOT authorize a deploy or N-3.** **N-2w commit-time approval (consumed by commit `e7bce599df678222ab0a58f0a4f3cc9f562f509a`) authorized only committing NEXT-ACTION.md; it did NOT authorize a deploy or N-3.** **N-2x commit-time approval, when granted, authorizes only committing the runbook + three status docs (Codex Q12 Path B §4(vii) addition + back-fills); it does NOT authorize a deploy, and it does NOT authorize N-3.** **All four prior production-action approvals (`9ae139d`, `3138e7f`, `e7bce59…` Attempt 3, `e7bce59…` Attempt 4) remain CONSUMED.**
...
- All four prior Victor production-action approvals (`9ae139d`, `3138e7f`, `e7bce599df678222ab0a58f0a4f3cc9f562f509a` Attempt 3, `e7bce599df678222ab0a58f0a4f3cc9f562f509a` Attempt 4) remain CONSUMED and cannot be reused for N-3.
...
The Migration 008 runbook `orchestrator/handoffs/N-2-MIGRATION-008-PRODUCTION-PLAN.md` IS part of the N-2x docs-only commit scope (Codex Q12 Path B §4(vii) addition + §1.1 Attempt-3/4 back-fill + §14 N-2v/N-2w/N-2x back-fill + §1/§9/§11 phase-label updates) and remains the canonical runbook for N-3 at the latest committed HEAD per `git rev-parse HEAD`.
