...
**Hands-Free ARC track — ARC-1 in progress (docs-only).** ARC-0 (read-only current-state verification) PASSed on 2026-05-02. ARC-1 (Orchestrator Protection Matrix) is active and docs-only: new `orchestrator/PROTECTED-FILES.md` drafted with per-path classification (SAFE / RESTRICTED / HARD BLOCK), Codex review and operator approval requirements, stop conditions, must-never-be-automated list, and explicit Ruflo / future-automation auto-approve prohibition. `orchestrator/APPROVAL-GATES.md` left unchanged (already canonical for action-class gating; cross-referenced from the new matrix). Minimal ARC-1 status entries added to `STATUS.md`, `CHECKLIST.md`, and this file. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migrations, no deploys, no live Kraken actions. No commit yet.** Pending Codex docs-only review and operator approval.

The next allowed action after this ARC-1 doc draft is: (a) Codex docs-only review of `orchestrator/PROTECTED-FILES.md` and the three minimal status-doc edits; (b) on Codex PASS, operator approval to commit; (c) commit `orchestrator/*` only — never stage or modify any HARD BLOCK file. After ARC-1 commit, the operator-chosen next phase below remains the operative path.
