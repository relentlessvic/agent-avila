...
| 10 | Any Ruflo install or automation upgrade (adding a new automation layer, widening an existing one, enabling new triggers, scheduling new agents). Includes Hermes install — see `orchestrator/COMM-HUB-HERMES-RULES.md` (SAFE-class) for the canonical Hermes spec, anti-execution boundaries, approval discipline, and staged activation path. | Explicit operator approval | Docs-only or design review of the automation-upgrade plan |
