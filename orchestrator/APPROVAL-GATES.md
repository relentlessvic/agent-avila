...
| 10 | Any Ruflo install or automation upgrade (adding a new automation layer, widening an existing one, enabling new triggers, scheduling new agents). Includes Relay install — see `orchestrator/COMM-HUB-RELAY-RULES.md` (SAFE-class; filename retains `HERMES` literal pending Phase B) for the canonical Relay spec, anti-execution boundaries, approval discipline, and staged activation path. | Explicit operator approval | Docs-only or design review of the automation-upgrade plan |
