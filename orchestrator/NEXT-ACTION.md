...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS (read-only audit), N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS (Codex), ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS (Codex; conversation-only), N-2b in progress (Phase N-2b — mode: DOCS-ONLY).** ARC-1 through ARC-7 closed in sequence. N-2 (Migration 008 production-application planning, DESIGN-ONLY) was delivered as a conversation-only design report; Codex returned PASS WITH REQUIRED EDITS with 5 tightening edits. **N-2b (DOCS-ONLY)** converts that conversation-only plan into a stable committed runbook artifact at `orchestrator/handoffs/N-2-MIGRATION-008-PRODUCTION-PLAN.md` with all five Codex-required edits applied verbatim: pre-flight (branch / HEAD / untracked-files / dirty-tree / `git ls-files` / target confirmation without secret), stabilized operator-approval wording, expanded verification (six indexes + full `schema_migrations` 1..8 + 13-column match), extended emergency stop (`applying`-log-line check + concurrent-runner detection), and extended risk table (two new P0 + two new P1 risks). No safety-policy doc is changed in N-2b. **N-3 (Migration 008 production application, mode PRODUCTION ACTION) remains hard-blocked** behind: Codex PASS on the runbook + explicit Victor in-session production-action approval per the runbook §5 wording + all pre-flight checks pass + operator confirms target Railway service / `DATABASE_URL` without exposing secret. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migration applied. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet.** Pending Codex docs-only review and explicit operator approval before any N-2b commit.

The next allowed action after this N-2b doc draft is: (a) Codex docs-only review of the runbook artifact (`orchestrator/handoffs/N-2-MIGRATION-008-PRODUCTION-PLAN.md`) + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2b commit, **N-3 remains hard-blocked** until the four §11 conditions in the runbook are simultaneously satisfied. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2b commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
