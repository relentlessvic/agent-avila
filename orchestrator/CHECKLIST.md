...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 in progress (docs-only).**
  ...
  - [~] **ARC-2 — Approval Gate Matrix (docs-only) — IN PROGRESS.**
    - [x] `orchestrator/APPROVAL-GATES.md` substantially expanded with: 16-gate numbered matrix (bot.js / db.js / migration file / production migration application / Railway deploy / live Kraken / live dashboard handler / position.json write / real-money / Ruflo install / Claude Code permission / production-state mutation / production env or secret / MANUAL_LIVE_ARMED / destructive git / automation widening); "actions that do NOT need operator approval" list; "what requires Codex review BEFORE operator approval"; "what an operator approval must include" structured form; "what is NOT operator approval" non-equivalence list; "production actions require separate explicit approval" separation rule; refreshed stop conditions; cross-references to `PROTECTED-FILES.md`. Original five sections (auto-allowed, HARD BLOCK list, in-between, Codex PASS authority, operator override) preserved.
    - [x] `orchestrator/PROTECTED-FILES.md` left unchanged (referenced as complementary per-path matrix; not rewritten).
    - [x] Minimal ARC-2 status entries added to `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, `orchestrator/NEXT-ACTION.md`.
    ...
    - [ ] Codex docs-only review of expanded `APPROVAL-GATES.md` and the three minimal status edits.
    - [ ] Operator approval to commit ARC-2 docs.
    - [ ] Commit ARC-2 docs (`orchestrator/*` only) after Codex PASS + operator approval.
