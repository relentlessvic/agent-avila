...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 in progress (Phase ARC-6 — mode: DOCS-ONLY).**
  ...
  - [~] **ARC-6 — Automation Permission Rules (Phase ARC-6 — mode: DOCS-ONLY) — IN PROGRESS.**
    - [x] New `orchestrator/AUTOMATION-PERMISSIONS.md` drafted as a safety-policy / control doc establishing the formal automation-permission rules for Claude / Codex / ChatGPT / Gemini / Ruflo / Hermes / future automation.
    - [x] Three-tier model defined: GREEN (safe automatic — read-only inspection, drafting docs, search, syntax checks) provided active phase mode allows it; YELLOW (requires Codex review — safety-policy doc edits, RESTRICTED file diffs, HIGH-RISK design reports, PRODUCTION ACTION plans, Claude Code permission changes); RED (requires explicit Victor / CEO approval and cannot be done automatically — bot.js / db.js / live dashboard handler edits, migration application, Railway deploys, live Kraken actions, position.json writes, real-money behavior, MANUAL_LIVE_ARMED actions, env/secret changes, destructive git, automation widening, Ruflo/Hermes install or upgrade, phase-mode promotion, master-order changes, safety-policy doc commits).
    - [x] Blocked-commands list enumerated: destructive git (`push --force`, `reset --hard`, `clean -f`, `rm`, `rebase`, `branch -D`, `restore .`), production deploy (`railway deploy`/`up`/`run` against prod), migration runner, production DB writes, position.json writes, runtime-file edits (`bot.js`/`db.js`/`migrations/`), env/secret writes, live Kraken triggers, automation widening (MCP install, hooks, slash commands, Ruflo/Hermes install).
    - [x] Stop conditions enumerated: RED-without-approval; blocked-command attempts; blocked-file modifications; Codex non-PASS treated as approval; non-operator-signal-as-approval; ambiguous mode; scoped-lift widening; unauthorized live Kraken / deploy / migration triggers; scheduled triggers firing YELLOW/RED actions; automation install/upgrade/expand attempts.
    - [x] Future-automation rules (Ruflo, Hermes, successors) explicitly inherit governance-only role; cannot become trading actors; cannot self-approve; cannot widen authority. Trading-runtime separation rule reinforced and bound by class (not by name).
    - [x] Cross-references to all six pre-existing safety-policy docs (PROTECTED-FILES.md, APPROVAL-GATES.md, PHASE-MODES.md, NEXT-ACTION-SELECTOR.md, ROLE-HIERARCHY.md, AUTOPILOT-RULES.md) plus BLUEPRINT.md, STATUS.md, NEXT-ACTION.md, CLAUDE.md. Conflict resolution: more restrictive rule wins.
    - [x] Registries updated: `orchestrator/PROTECTED-FILES.md` (3 places), `orchestrator/APPROVAL-GATES.md` (2 places), and `orchestrator/PHASE-MODES.md` (2 places) register `AUTOMATION-PERMISSIONS.md` as a safety-policy doc.
    - [x] ARC-6 status entries added to `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, `orchestrator/NEXT-ACTION.md`.
    ...
    - [ ] Codex docs-only review of the ARC-6 docs-only diff across `AUTOMATION-PERMISSIONS.md`, `PROTECTED-FILES.md`, `APPROVAL-GATES.md`, `PHASE-MODES.md`, `STATUS.md`, `CHECKLIST.md`, and `NEXT-ACTION.md`.
    - [ ] Operator approval to commit ARC-6 docs.
    - [ ] Commit exactly those seven ARC-6 docs after Codex PASS + operator approval.
