...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-2c closed (`9ae139d`), N-3 attempt-1 2026-05-03 HALTED at HEAD `9ae139d` (DNS / execution-context failure; Migration 008 NOT applied; approval consumed), N-2d closed (`3732721`), N-2e closed (`afe94d1`), N-2f closed (`3af1e44`), N-2g closed (`926eb7f`), N-2h closed (`ea7774d`), N-2i closed (`5ee1dcb`), N-2j closed (`548383b`), N-2k closed (`b2d187d`), N-2l closed (`3138e7f`), **N-3 attempt-2 2026-05-03 HALTED at HEAD `3138e7f`**, N-2m closed (`6b9be1d`), **Codex N-2n design review on Strategy A returned FAIL** (Railway CLI run-mode / shell-mode inject env vars only, do NOT proxy private-network DNS), N-2n in progress (Phase N-2n — mode: DOCS-ONLY).** ARC-1 through ARC-7 closed in sequence. N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h / N-2i / N-2j / N-2k / N-2l established, stabilized, connectivity-preflight-extended, N-3-preflight-tightened, DDL-baseline + rollback-blocker-tightened, runtime-identity + DB-baseline-tightened, §14 stale-tail-loop-fixed, self-hash-loop-removed + runtime-drift-tightened, Node-runtime-exact-version-tightened, Node-pin-conflict-resolution-tightened, and Node-pin-whitespace-normalization-tightened the production-plan runbook. The single N-3 attempt on 2026-05-03 HALTED before any SQL executed because the runner was invoked from a non-Railway-attached execution context. Production-DB state is unchanged. Migration 008 is still NOT applied. The prior Victor approval was consumed by the failed attempt and cannot be reused. A second N-3 production-application attempt at approved HEAD `3138e7f` HALTED on 2026-05-03 with the **identical** `getaddrinfo ENOTFOUND postgres.railway.internal` failure class as the first attempt at HEAD `9ae139d` — runner exit 1, no `applying` log line, no `BEGIN`/`COMMIT`, Migration 008 NOT applied, production-DB unchanged, Victor approval naming `3138e7f` consumed by the failed attempt and not reusable. **The failure occurred despite a §4(vii) connectivity preflight PASS report one turn before runner invocation**, strongly indicating context drift between the preflight check and the runner invocation. **N-2m (DOCS-ONLY)** documents this second HALT in §1.1 and tightens §4(vii) with mandatory same-session / no-context-switch + actual-runner-DNS-path requirements: the (a)–(d) checks must be performed from the EXACT SAME execution context that will invoke the runner (same shell session, same Railway CLI scope, same network namespace, same working directory, same Node / OS process namespace); no context switch permitted between (vii) PASS and runner invocation; the (a) DNS and (c) auth checks must exercise the actual Node-process resolver path, not dashboard reachability or a different CLI tool. §4(x) gets a same-context-alignment cross-reference. §7 gets a HALT-after-claimed-§4(vii)-PASS rule that returns N-3 to docs-only / design-only review (no operational iteration within the same attempt window). §8 gets two new rows (P0 context-drift; P1 different-DNS-path). §11 condition (1) updated to N-2m. §9 gate-separation chain extended through N-2m. **Both Victor approvals (`9ae139d`, `3138e7f`) are CONSUMED.** No §4(i)–(vi) / §4(viii)–(ix) / §4(x)(b) / §4(xi) / §5 / §6 / §11 substance changed; §4(vii) and §4(x) context-alignment are strengthened, §7 and §8 are extended. Pre-flight remains at eleven checks (i)–(xi); no new check added. The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` values, no hostname/port/URL values added. **N-3 remains halted/blocked.** No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` / `package.json` / lockfiles. No retry. No rollback. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet. Pending Codex docs-only review and explicit operator approval before any N-2n commit.

(Prior phase N-2l: Node exact-pin whitespace-normalization. The fresh Codex N-3 preflight review on the N-2k-committed runbook (HEAD `b2d187d`) had returned FAIL with one finding: §4(x)(b) was SILENT on whitespace / newline normalization for Node-pin source values — a standard Unix `.nvmrc` file ending with `\n` would fail a literal byte-for-byte runtime match unless normalization was specified, producing a false-FAIL on a structurally-correct setup; conversely, an unspecified normalization would let implementation choice silently affect the safety boundary. **N-2l (DOCS-ONLY)** adds an explicit parsing-hygiene rule to §4(x)(b): every Node-pin source value AND the runtime version output is normalized by stripping leading whitespace, trailing whitespace, and trailing newlines (and the leading `v` from the runtime `node --version` output) before any exactness test is applied. After trimming, the value must still be a literal `major.minor.patch` form — trimming does NOT relax rejection of aliases (`lts/foo`, `node`, `latest`), semver ranges, pre-release identifiers (e.g., `20.18.5-rc1`), build metadata (e.g., `20.18.5+abc`), a leading `v` in a source-value file, or any other non-`major.minor.patch` form. §8 P1 runner-version-drift risk description and mitigation rewritten to match. The N-2k canonical-source + HALT-on-disagreement + any-source-PASS-prohibited rules remain unchanged. §11 condition (1) updated to N-2l. §9 gate-separation chain extended through N-2l. No §4(i)–(ix) / §4(x)(a) / §4(x)(c) / §4(xi) / §5 / §6 / §7 substance changed in N-2l other than §11 / §9 phase-label updates. Pre-flight remains at eleven checks (i)–(xi); no new check added. No `package.json` / lockfile / runtime-config file is changed in N-2l (the fix is docs-only — the runbook now defines how the operator handles whitespace / newline normalization in Node-pin source values). The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` values, no hostname/port/URL values added. **N-3 remains hard-blocked** behind a fresh five-condition gate: (1) Codex docs-only PASS on this N-2l update; (2) fresh Codex N-3 preflight PASS on the now-updated runbook; (3) fresh Victor explicit in-session production-action approval naming the exact current HEAD at retry time; (4) all eleven pre-flight checks (i)–(xi) PASS at execution time; (5) target Railway service / production database confirmation without exposing secrets. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` / `package.json` / lockfiles. No migration applied. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet.** Pending Codex docs-only review and explicit operator approval before any N-2l commit.

The next allowed action after this N-2n doc draft is: (a) Codex docs-only review of the runbook diff + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit N-2n; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2n commit, **N-3 remains halted/blocked** behind the five-condition gate, with both prior Victor approvals (`9ae139d` and `3138e7f`) already consumed. Any future N-3 attempt requires fresh Codex N-3 preflight PASS at the new HEAD, fresh Victor approval naming the new HEAD, AND operator-side execution against the new approved primary surface — a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container (per N-2n's §3 + §4(vii) gating), NOT a local `railway run` / `railway shell` session. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2n commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h / N-2i / N-2j / N-2k / N-2l / N-2m / N-2n / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
...
> **N-2n Codex docs-only review of the Railway execution-surface correction runbook update, then explicit operator approval before any N-2n commit; N-3 remains halted/blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2n, fresh Codex N-3 preflight PASS, fresh Victor approval naming a NEW HEAD — both `9ae139d` and `3138e7f` approvals are consumed, all eleven pre-flight checks (i)–(xi) PASS at execution time from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container per the new §3 + §4(vii) gating, target confirmation without exposing secrets).**
...
- Before applying Migration 008: follow the current N-2n runbook gate — fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD (note: both prior approvals at `9ae139d` and `3138e7f` are consumed), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time **from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container** (the approved primary execution surface per N-2n; local Railway CLI run-mode / shell-mode are explicitly REJECTED for internal-hostname `DATABASE_URL`), target confirmation without exposing secrets, then the full §6 verification after runner exit.
...
For **Migration 008 production-application planning** (when authorized): the operator-driven sequence is — (1) satisfy the current N-2n runbook gate, including fresh Codex N-3 preflight PASS, fresh Victor approval naming a new HEAD (both prior approvals at `9ae139d` and `3138e7f` are consumed by HALTED attempts), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container (the approved primary execution surface per N-2n; local Railway CLI run-mode / shell-mode are explicitly REJECTED), and target confirmation without exposing secrets; (2) apply Migration 008 only under that explicit production-action approval; (3) complete the full §6 post-application verification; (4) document the application timestamp in `STATUS.md`. After application, D-5.12d/e production deployment is unblocked (still requires its own explicit operator authorization).
