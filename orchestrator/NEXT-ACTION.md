...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-3 preflight review PASS WITH REQUIRED EDITS (Codex; stale pinned HEAD caught), N-2c in progress (Phase N-2c — mode: DOCS-ONLY).** N-2b committed the production-plan runbook at `e6c9189` (14 sections, 185 lines, full HANDOFF-RULES.md prose-only compliance). **Codex N-3 preflight review** caught a stale-pinned-HEAD bootstrap problem: the runbook still pinned pre-flight check (i) HEAD to `8266db2…` (the pre-N-2b commit), but the runbook now lives at `e6c9189`. Naively updating the pin to `e6c9189…` would re-stale on the next commit (self-staling commit loop). **N-2c (DOCS-ONLY)** replaces the in-runbook hardcoded HEAD pin with an approval-time exact-HEAD-naming rule: pre-flight check (i) now requires HEAD to equal the exact hash Victor names in his N-3 approval; §5 adds an exact-HEAD-naming requirement; the risk table is reformulated to require HALT on HEAD-change after approval. **N-3 remains hard-blocked** behind: fresh Codex N-3 preflight PASS on the corrected runbook + Victor explicit in-session production-action approval **naming the exact current HEAD hash returned by `git rev-parse HEAD`** + execution-time pre-flight pass + target Railway service / production `DATABASE_URL` confirmation without exposing secret. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migration applied. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet.** Pending Codex docs-only review and explicit operator approval before any N-2c commit.

The next allowed action after this N-2c doc draft is: (a) Codex docs-only review of the runbook diff + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2c commit, **N-3 remains hard-blocked** until all four §11 conditions in the corrected runbook are simultaneously satisfied. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2c commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
...
> **N-2c Codex docs-only review of the corrected runbook, then explicit operator approval before any N-2c commit; N-3 remains hard-blocked behind separate explicit Victor approval naming the exact current HEAD.**
