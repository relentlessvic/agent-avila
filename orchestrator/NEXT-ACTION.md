...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 in progress (Phase ARC-4 — mode: DOCS-ONLY).** ARC-0 (read-only current-state verification) PASSed on 2026-05-02. ARC-1 (Orchestrator Protection Matrix) closed; committed `95157ae`. ARC-2 (Approval Gate Matrix) closed; committed `3b53223`. ARC-3 (Phase Mode System) closed with Codex final PASS and operator approval; committed `9b5093f` (6 files, +258 / -17). ARC-4 (Next-Action Selector) is now active and docs-only: new `orchestrator/NEXT-ACTION-SELECTOR.md` is a safety-policy doc enforcing strict ordering of future Agent Avila phases. Selector defines ten ordered rules and produces a current master order (ARC-4 → ARC-5 → ARC-6 → N-1 → N-2 → N-3 → N-4) with a hard rule that D-5.12f cannot start until ARC-4 / ARC-5 / ARC-6 are closed unless the operator explicitly changes the master order. Registries updated: `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, and `orchestrator/PHASE-MODES.md` register `NEXT-ACTION-SELECTOR.md` as a safety-policy doc. ARC-4 status entries added to `STATUS.md`, `CHECKLIST.md`, and this file. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migrations, no deploys, no live Kraken actions. No commit yet.** Pending Codex docs-only review and explicit operator approval (NEXT-ACTION-SELECTOR.md is a safety-policy doc).

The next allowed action after this ARC-4 Path A doc draft is: (a) Codex docs-only review of `orchestrator/NEXT-ACTION-SELECTOR.md`, `orchestrator/PHASE-MODES.md`, `orchestrator/PROTECTED-FILES.md`, `orchestrator/APPROVAL-GATES.md`, `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit; (c) commit exactly those seven paths only — never stage or modify any HARD BLOCK file. After ARC-4 commit, the selector advances to ARC-5 → ARC-6 → N-1 → N-2 → N-3 → N-4 per `orchestrator/NEXT-ACTION-SELECTOR.md`.
