# Communication Hub — Hermes Install Checklist (template — COMM-HUB)

> **Author rule:** This checklist codifies the click-by-click manual Hermes install plan for the Agent Avila Communication Hub. It is a docs-only specification — not a runtime install. **This checklist is NOT authorization to install Hermes, register a Discord application or bot, mint a bot token, invite a bot to the server, grant any permission, install a webhook / scheduler / MCP trigger / cron job / Ruflo / background automation, change CEILING-PAUSE, take any production action, take any trading action, or post to Discord.** Actual Hermes install requires a separate operator-approved phase (`COMM-HUB-HERMES-INSTALL`, Stage 5 per `orchestrator/COMM-HUB-HERMES-RULES.md`) which is **Gate 10 (automation install / upgrade)** per `orchestrator/APPROVAL-GATES.md` and requires explicit Victor in-session approval at that future time.
>
...
- `orchestrator/handoffs/COMM-HUB-CHANNEL-LAYOUT.md` — canonical channel/role/permission layout (lines 136–145 carry `System-Writer (Hermes)` DORMANT row)
...
If any field below diverges from `orchestrator/COMM-HUB-HERMES-RULES.md`, `orchestrator/COMM-HUB-RULES.md`, or `orchestrator/handoffs/COMM-HUB-CHANNEL-LAYOUT.md`, the canonical files win and this checklist must be re-aligned in a follow-up DOCS-ONLY phase. The Hermes spec is canonical for capability scope; the rulebook is canonical for the Communication Hub; the channel layout is canonical for per-role permissions.

---

## Phase context

This checklist is the persistent on-disk codification of the Stage 3 Hermes install plan, downstream of the Stage 1 design (`COMM-HUB-DESIGN-HERMES`, Codex 8/8 PASS after one revision round; conversation-only) and Stage 2 codification (`COMM-HUB-DOCS-C-HERMES-SPEC`, closed at `96f56a4767cc96ddd8b78bcc3b309e8fd455c8a5`; Hermes spec written; cross-references applied).
...
**Hermes remains DORMANT (zero members, zero permissions) throughout all stages prior to Stage 5 install completion. After install, Hermes remains constrained by its capability allow-list (Send Messages + View Channels for 3 channels) regardless of stage.**

---

## Preconditions before Hermes install (must all be true before Stage 5 opens)
...
2. **Server install verified.** Verification checklist in the Discord install checklist passed; channel structure matches the canonical 3-category / 7-channel layout; Future-Trading remains DORMANT; no bot / webhook / scheduler / MCP / cron / Ruflo / Hermes / background automation is currently installed.
3. **Hermes spec is canonical and unchanged.** `orchestrator/COMM-HUB-HERMES-RULES.md` is the canonical Hermes spec; no divergence from `orchestrator/handoffs/COMM-HUB-CHANNEL-LAYOUT.md`.
...
6. **CEILING-PAUSE state.** Active and not broken; or explicit Victor direction-confirmation has reset the counter to 0 and authorized this Hermes install track. Operator-directed manual stages do NOT advance the autopilot phase-loop counter and do NOT break CEILING-PAUSE.
...
12. **Operator hosting decision recorded.** The host environment for the Hermes process has been chosen by Victor (e.g., a separate Railway service, a separate dedicated container, a self-hosted box; never the `agent-avila-dashboard` service); the choice is documented in the Stage 5 packet. **Hermes does NOT share a process, image, or env scope with the trading runtime.**
13. **Operator network-allowlist plan recorded.** The egress allowlist for the Hermes host (Discord API only) has been documented in the Stage 5 packet.
...
| Halt-resume approval | Any time Hermes halts | Per-action | No auto-resume |
| Deactivation approval | `COMM-HUB-HERMES-DEACTIVATE` (always available) | Per-action | Reverts Hermes to DORMANT |

The set of approvers is and remains exactly `{Victor}`. Hermes never has approval authority.
...
2. **Create a new application.** Developer Portal → Applications → New Application → name: `Agent Avila Hermes` (or operator-chosen equivalent matching the spec). Confirm 2FA prompt if Discord requests it.
...
6. **Disable all Privileged Gateway Intents.** Application → Bot → Privileged Gateway Intents: **Presence Intent OFF**, **Server Members Intent OFF**, **Message Content Intent OFF**. Hermes does not need any privileged intent. **No intent is enabled at install.**
...
14. **Set up the host process.** On the operator-chosen host (precondition 12), provision the Hermes container/image with the Discord token from the secret store. The host MUST: (a) restrict egress to the Discord API only (precondition 13 allowlist), (b) run the Hermes process as a non-root, non-privileged user, (c) have no git checkout, (d) have no DB credentials, (e) have no Kraken credentials, (f) have no Railway credentials beyond what the host requires for its own management, (g) have no `MANUAL_LIVE_ARMED` env var, (h) have only the Discord bot token + a logging endpoint env var.
15. **Configure idempotency store.** Provision the Hermes-private append-only publish log on the host (file or simple append-only datastore). Verify the log location is operator-readable, append-only, and not shared with any other process.
16. **Configure source-of-truth message store.** Provision the read-only append-only message store the Hermes process reads from (per Hermes spec §"Message contract / metadata schema"). At install, the store is initially empty; first messages are added in Stage 8 / 9 only.
17. **Configure halt-on-anomaly defaults.** Hermes process configuration: halt-on missing/stale Codex PASS metadata, halt-on missing operator authorization, halt-on channel not in allow-list, halt-on character limit exceeded, halt-on rate limit hit, halt-on network anomaly, halt-on idempotency-key mismatch, halt-on CEILING-PAUSE state ACTIVE. **No auto-resume.**
18. **Smoke-test connectivity (no publish).** Operator runs a connection-only check: Hermes process boots, authenticates to Discord with the bot token, lists allowed channels via the View Channels permission (read-only listing of channel ids; NOT message content), confirms the 3 allowed channels are reachable and the 4 forbidden channels are NOT in the bot's visible-channel list (because of step 13 deny overrides). **No message is published in this smoke test.** If any anomaly, halt and re-run earlier steps.
19. **Stop the Hermes process after smoke-test.** The process remains in DRAFT-ONLY (Stage 8) state by default until the Stage 7 dry-run / Stage 8 draft-only-mode / Stage 9 first-auto-publish approvals each open separately.
...
- Connecting Hermes to Railway, Kraken, the production DB, GitHub, or any non-Discord endpoint.

---

## Permission boundaries (full enumeration)

Hermes role permissions at install time, mirroring `orchestrator/COMM-HUB-HERMES-RULES.md` Allowed/Forbidden capability matrix.
...
| `Read Message History` | **OFF (forever — explicit non-listener clause)** | No Discord-side reads for any purpose; idempotency uses orchestrator-side keys + Hermes-private append-only publish logs |
...
| Hermes-private append-only publish log | APPEND-ONLY |

### Approval / phase / autopilot boundaries

| Boundary | State |
|---|---|
| Approval authority | NONE forever (Hermes is governance-only) |
| Phase open / close authority | NONE |
| Autopilot input / output | NONE (Hermes is not an autopilot input or output) |
| Self-modification | NONE (Hermes cannot modify its own permissions, the spec, the channel layout, the install checklist, or any orchestrator doc) |
| Auto-resume after halt | NONE |
| CEILING-PAUSE break | NONE (Hermes detects state and halts during it) |

---

## Channels Hermes may eventually post to (3 of 7)

After full activation through Stage 10b, Hermes write authority is restricted to these 3 channels:

| Channel | Hermes write authority earliest stage | Per-message Victor approval | Class authorization eligible? |
|---|---|---|---|
...
**Hermes write authority is NOT granted at Stage 5 install or Stage 6 closeout.** Stage 5 grants only the role + channel-level allow overrides for the 3 channels; Stage 7 dry-run produces no real Discord post; Stage 8 draft-only-mode produces drafts but no publish; Stage 9 is the FIRST stage that authorizes any real auto-post, and only to `#status`, and only with per-message Victor approval for every message.

## Channels Hermes must NOT read (full enumeration)

Hermes does NOT read any Discord channel for any purpose. The non-listener clause is explicit and mandatory.

| Channel | Hermes read authority |
|---|---|
| `#approvals` | **NEVER** |
| `#status` | **NEVER** (Hermes posts to `#status` write-only; never reads message history; idempotency uses orchestrator-side keys + Hermes-private logs) |
...
> **`Read Message History` is NEVER granted to Hermes. It is OFF at the role level (step 11), and is **explicitly denied** via per-channel overrides on every channel (steps 12 and 13). Hermes does not read Discord channels for idempotency, deduplication, monitoring, replies, reactions, audit, or any input interpretation. Idempotency MUST be enforced using orchestrator-side idempotency keys in the approved message metadata plus Hermes-private append-only publish logs. If the idempotency key is missing, reused inconsistently, or unverifiable, Hermes halts without posting.**

If a future operator-directed manual phase ever needs to grant `Read Message History` to Hermes, it must be a separately-scoped Gate-10 phase with its own design, Codex review, and explicit Victor in-session approval naming the exact channel and exact use case. **No such phase is currently planned.** Until and unless that future phase opens, Hermes remains a one-way write-only publisher with no Discord-side reads.

---

## No trading authority

Hermes has zero trading authority forever. Specifically:
...
- **Posting to Category C (`#trading-alerts`, `#trading-summaries`) is NEVER authorized for Hermes.** Trading-Writer is a different role; multi-gated activation is required for that role; Hermes never posts in Category C.

## No approval authority

Hermes has zero approval authority forever. Specifically:

- Hermes never grants, denies, or interprets approvals.
...
- Hermes never opens or closes orchestrator phases.
- Hermes never breaks CEILING-PAUSE (it detects state and halts during it).
- Hermes is never an autopilot input or output.
- Hermes never authorizes its own future activations or expansions.

## No automatic posting until future approved stages

Hermes does NOT auto-post at install (Stage 5) or at closeout (Stage 6). The first authorized auto-post lands at Stage 9 only.

| Stage | Hermes posting state |
|---|---|
...
| Stage 8 draft-only-mode | No publish (Hermes generates drafts; orchestrator + Codex + operator manual post still required) |
...
The Hermes-private log location, append-only enforcement, and operator-readability are configured at install step 15. The log is **never** posted to Discord, **never** committed to the repo, **never** shared externally beyond operator review.
...
Hermes can be reverted to DORMANT at any stage. The operator-directed manual `COMM-HUB-HERMES-DEACTIVATE` phase performs the rollback. The phase is operator-approved per-action and produces no automated cascade.

**Rollback steps:**

1. **Stop the Hermes process.** On the host, terminate the Hermes process (operator-chosen mechanism per host; does not require Discord-side action).
...
3. **Remove the bot from `Agent Avila Hub`.** Server Settings → Members → `Agent Avila Hermes` bot → Kick. The bot is removed from the server immediately.
...
6. **Wipe the host-side secrets.** Remove the Discord bot token from the host's secret store. Remove any Hermes-related env var.
7. **Archive the Hermes-private publish log.** Copy the log to a local operator-only archive; **do NOT post or commit it.** Optionally delete the live log on the host once archived.
...
11. **Document rollback completion** in a separate operator-approved phase (an explicit `COMM-HUB-HERMES-DEACTIVATE-CLOSEOUT` or equivalent) by updating STATUS / CHECKLIST / NEXT-ACTION docs. Hermes returns to canonical DORMANT (zero members, zero permissions) state.

**Rollback explicitly does NOT:**

- Modify the Hermes spec (`orchestrator/COMM-HUB-HERMES-RULES.md`).
...
Operator confirms each item with a direct visual check in the Discord client / developer portal / host shell. Each item is explicitly visible to the operator without requiring `Read Message History` on Hermes (since Hermes never has that permission).

- [ ] Discord application named `Agent Avila Hermes` (or operator-chosen) exists in Victor's developer portal.
...
- [ ] Hermes-private append-only publish log location is configured and operator-readable.
...
- [ ] Smoke-test connection check passed (Hermes booted, authenticated, listed allowed channels by id without reading message content; the 4 forbidden channels are NOT in the bot's visible-channel list); **no message was published** during smoke-test.
- [ ] Hermes process is in stopped state at the end of install (will be re-started only at Stage 7 dry-run / Stage 8 draft-only-mode / Stage 9 first-auto-publish under separate operator approval).
- [ ] `Agent Avila Hub` server still has zero webhooks, zero other bots, zero third-party integrations beyond Hermes (Server Settings → Integrations confirms).
...
- [ ] Server Settings → Integrations (showing Hermes is the only bot; zero webhooks; zero third-party integrations).
...
- [ ] Hermes-private publish log location capture (empty log at install).
...
2. The Hermes spec (`orchestrator/COMM-HUB-HERMES-RULES.md`) is unchanged from its canonical HEAD (`96f56a4767cc96ddd8b78bcc3b309e8fd455c8a5`) or has been re-reviewed if changed.
...
9. CEILING-PAUSE state ACTIVE-NOT-BROKEN (or explicit Victor direction-confirmation has reset the counter to 0 and authorized this Hermes install track).
...
Per-message Codex pre-publish sanity check is canonical at the Communication Hub level (`orchestrator/COMM-HUB-RULES.md`) and applies to every Hermes auto-publish at Stage 9 / 10a / 10b.
...
- **R10** — Hermes process granted access to Kraken / Railway / production DB / GitHub / `MANUAL_LIVE_ARMED` env. Mitigation: step 14 host-process boundaries explicitly forbid those env vars; verification checklist confirms.

**P2 (medium impact):**

- **R11** — Operator stores Hermes-private publish log in a location that is also a git checkout, leading to accidental commit of Discord activity records. Mitigation: step 15 specifies the log is on the Hermes host, not the operator's git working tree.
...
- **R13** — Hermes process auto-restarts after halt due to host's default container restart policy. Mitigation: step 17 halt-on-anomaly is explicit; host configuration must disable container auto-restart on Hermes process exit (operator records in install evidence).
...
- **R15** — Idempotency key collision between two orchestrator drafts with the same key. Mitigation: orchestrator-side key generation must be globally unique (e.g., timestamp + phase id + counter); duplicate detection in Hermes triggers halt-on-anomaly.
...
- **R17** — Hermes-private publish log truncated or lost. Mitigation: append-only file with operator-managed retention; backup discipline is operator-defined; lost-log halts new publishes (idempotency cannot be verified) until operator restores.
- **R18** — Operator unintentionally posts a draft message in `#status` while Hermes is also configured to post the same message (race condition between operator manual post and Hermes auto-publish). Mitigation: Stage 9 onward, manual-post-vs-auto-post boundary is documented per-stage; idempotency key + halt-on-anomaly catches duplicate publishes; pre-Stage-9 there is no Hermes publish authority so no race exists.
...
- **R22** — Future Hermes spec change reveals new permission gap. Mitigation: this checklist has a "canonical files win" rule; spec change goes through a successor `COMM-HUB-DOCS-X-HERMES-SPEC` phase; install checklist re-runs against new spec before any change to live Hermes.
...
- **R24** — Token rotation cadence not followed. Mitigation: operator-defined rotation schedule documented at install; rotation does not require code change in Hermes; only token change.
...
- Hermes-to-`#approvals` posting (forever forbidden; not a future phase).
- Hermes-to-`#codex-warnings` posting (forever forbidden; not a future phase).
- Hermes-to-Category-C posting (forever forbidden for Hermes; Trading-Writer is a different role).
- Cross-server Hermes (Hermes is restricted to `Agent Avila Hub` only).
- Anonymous / public install of Hermes (forever forbidden).

---

## Hard limits

- No Hermes runtime is installed by writing this checklist or by following it.
...
- No widening of Hermes' capability matrix beyond `Send Messages` + `View Channels` for the 3 allowed channels.
- No grant of `Read Message History` to Hermes.
- No grant of any approval authority to Hermes.
- No grant of trading authority to Hermes.
- Hermes is not connected to Railway, the production DB, Kraken, GitHub, or any non-Discord endpoint.

---

## What this checklist is NOT

- **Not authorization to install Hermes.** Hermes install is Stage 5 (`COMM-HUB-HERMES-INSTALL`) — Gate 10 RED-tier per `orchestrator/APPROVAL-GATES.md` and requires explicit Victor in-session approval at that future time.
...
- **Not canonical over `orchestrator/COMM-HUB-HERMES-RULES.md`.** If this checklist diverges from the Hermes spec, the spec wins.
...
**This phase (`COMM-HUB-DOCS-D-HERMES-INSTALL-CHECKLIST`) is DOCS-ONLY and does NOT activate Hermes. Hermes remains DORMANT (zero members, zero permissions) at the end of this phase. Stage 5 install requires its own separately-approved Gate-10 phase.**
