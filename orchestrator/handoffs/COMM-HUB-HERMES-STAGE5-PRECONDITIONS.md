...
- `orchestrator/handoffs/COMM-HUB-INSTALL-HERMES-CHECKLIST.md` — Hermes Stage 5 install checklist (15 preconditions; this document codifies preconditions 12–15)
...
This Stage 4-related codification phase (`COMM-HUB-DOCS-F-HERMES-STAGE5-PRECONDITIONS`) is **DOCS-ONLY**. It does NOT install Hermes, register a Discord application, mint a Discord bot token, invite a bot, grant any permission, install a webhook / scheduler / MCP trigger / cron job / Ruflo / Hermes runtime / background automation, post to Discord, take a production action, take a trading action, or break CEILING-PAUSE.
...
Hermes must run in a **separate isolated host / service / container** from the trading runtime. The Hermes process MUST NOT share any of the following with the trading runtime (`bot.js`, `dashboard.js`, the production DB, the production Railway service `agent-avila-dashboard`, or any other live trading-path component):
...
The following are explicitly forbidden as the Hermes host:
...
- Any local developer workstation that has live Kraken credentials or production-DB credentials (would couple operator-side work to Hermes runtime).
...
|---|---|---|
| Separate Railway service | A new Railway service (separate from `agent-avila-dashboard`) provisioned exclusively for Hermes | Must enforce no-shared-env; Railway-side firewall / egress allowlist required |
...
| Serverless function (e.g., Cloudflare Worker / AWS Lambda) bound to Hermes scope only | Limited; depends on Discord API client compatibility | Must enforce egress allowlist; no shared env beyond bot token |
...
- **No-shared-env confirmation:** "Hermes host does not share process / image / volume / env scope / Railway service id / DB / Kraken / `MANUAL_LIVE_ARMED` / GitHub deploy hook / CI/CD secret access with the trading runtime."
- **No-shared-DB confirmation:** "Hermes host has no `DATABASE_URL` env var or any DB connection string."
- **No-shared-Kraken confirmation:** "Hermes host has no `KRAKEN_API_KEY` / `KRAKEN_API_SECRET` env var or any Kraken endpoint reachable from its egress."
- **No-trading-runtime-import confirmation:** "Hermes host does not import / reference any trading-runtime file (`bot.js`, `dashboard.js`, `db.js`, `migrations/`, `scripts/`, `position.json`)."
...
Hermes egress (outbound network) is restricted to **Discord API endpoints only**, plus required DNS / TLS resolution if necessary for those endpoints to function.

### Allowed egress endpoints (full enumeration)

| Endpoint class | Purpose | Notes |
|---|---|---|
| Discord API gateway endpoint(s) | Hermes gateway authentication (`IDENTIFY` / `READY`) | Required for Send Messages + View Channels |
| Discord REST API endpoint(s) | Hermes `Send Message` calls (only when in non-dry-run mode); `View Channels` (read-only channel-list inspection) | Required for the bot's allow-listed scopes |
...
The following endpoints / endpoint classes MUST NOT be reachable from the Hermes host:
...
- **Forbidden-egress confirmation:** "Hermes host egress is restricted to Discord API endpoints only. Kraken, Railway, production DB, GitHub, CI/CD, other LLM providers, and arbitrary internet egress are blocked."
- **Halt-on-anomaly confirmation:** "Hermes process is configured to halt-on-anomaly (per the canonical halt-on-anomaly mechanism) on any egress attempt to a non-allow-listed endpoint."
...
The Discord bot token MUST live **only in the selected host's secret store**. The token is the credential that authorizes Hermes to authenticate to Discord; leakage of the token would allow any party to impersonate Hermes. Strict storage discipline is required.
...
| Token logged in any operator-readable log | Tokens redacted in every log; the Hermes-private append-only publish log records the bot's actions but NOT the token value itself |
...
| Discord developer portal access | Required (Victor will register the Hermes application via the developer portal at Stage 5) |
...
- Registers the Hermes application in the Discord developer portal.
- Authorizes the Hermes bot's invitation to `Agent Avila Hub`.
...
- **"I will register the Hermes application using my primary Discord account; the Hub-Read alt account (if assigned) will NOT perform any Stage 5 install action."**
...
- **Hermes DORMANT** (zero members, zero permissions; canonical Hermes spec and install checklist roles unchanged).
- **Stage 5 `COMM-HUB-HERMES-INSTALL` RED-tier Gate-10** per `orchestrator/APPROVAL-GATES.md` line 56.
- **Stage 7 `COMM-HUB-HERMES-DRY-RUN` separately gated** (per the canonical Hermes spec staged-path table; not authorized by Stage 5 install approval).
...
- Hermes install (Stage 5 — `COMM-HUB-HERMES-INSTALL`).
...
- Modification of the canonical Hermes spec, install checklist, dry-run design, channel layout, or any safety-policy doc.
...
- **Not authorization to install Hermes.** Hermes install is Stage 5 — Gate-10 RED-tier per `orchestrator/APPROVAL-GATES.md` and requires explicit Victor in-session approval at that future time, after Codex install-readiness PASS.
