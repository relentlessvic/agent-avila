...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS (read-only audit), N-1 closeout sync in progress (mode: DOCS-ONLY).**
  ...
  - [~] **N-1 closeout sync (mode: DOCS-ONLY) — IN PROGRESS.**
    - [x] Mark ARC-6 as closed / committed (`9eff3c9`) in STATUS.md, CHECKLIST.md, NEXT-ACTION.md.
    - [x] Mark N-1 Repo Confirmation as passed in STATUS.md, CHECKLIST.md, NEXT-ACTION.md.
    - [x] Remove stale wording that ARC-6 is still in progress / awaiting Codex review / awaiting operator approval / awaiting commit.
    - [x] Set "Current allowed next action" pointer to N-2 (Migration 008 production-application planning, mode DESIGN-ONLY).
    - [x] State explicitly that Migration 008 exists on disk, production-application status remains NOT live-DB verified, and verification / application requires separate production-action planning (N-2) and explicit operator approval (N-3).
    - [x] Preserve hard-block on D-5.12f until N-2 / N-3 are handled per the selector unless the operator explicitly changes the master order.
    - [x] No edits to safety-policy docs (PROTECTED-FILES.md, APPROVAL-GATES.md, PHASE-MODES.md, NEXT-ACTION-SELECTOR.md, ROLE-HIERARCHY.md, AUTOMATION-PERMISSIONS.md, AUTOPILOT-RULES.md, BLUEPRINT.md). Edits confined to STATUS.md, CHECKLIST.md, NEXT-ACTION.md.
    - [x] No edits to `bot.js`, `dashboard.js`, `db.js`, `migrations/`, `scripts/`, or `position.json`.
    - [x] No migrations applied. No deploys. No live Kraken actions. No production-DB queries.
    - [ ] Codex docs-only review of the N-1 closeout sync diff across `STATUS.md`, `CHECKLIST.md`, and `NEXT-ACTION.md`.
    - [ ] Operator approval to commit the N-1 closeout sync.
    - [ ] Commit exactly those three docs after Codex PASS + operator approval.
