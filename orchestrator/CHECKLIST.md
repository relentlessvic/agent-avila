...
- [ ] ARC-8-RUN-B: STATUS.md updated to record ARC-8-RUN-A-CLOSEOUT closed at `a427d1c…`, phase table extended with ARC-8-RUN-B row, ARC-8 Activation section rewrite for ARC-8-RUN-B framing, Closeout Note rewrite, Current Phase State subsection updated.
- [ ] ARC-8-RUN-B: CHECKLIST.md updated (this section), phase table close ARC-8-RUN-A-CLOSEOUT / open ARC-8-RUN-B, ARC-8-RUN-A-CLOSEOUT 10 [ ] checkboxes converted to [x] (this is one of the 10).
- [ ] ARC-8-RUN-B: NEXT-ACTION.md updated with phase chain extension, current-phase summary rewrite for ARC-8-RUN-B, Required Gate State extended with ARC-8-RUN-A-CLOSEOUT consumed clause + ARC-8-RUN-B clause.
- [ ] ARC-8-RUN-B: scope confirmed exactly the 3 status docs (STATUS.md, CHECKLIST.md, NEXT-ACTION.md); no other file modified by this phase.
- [ ] ARC-8-RUN-B: ARC-8-DOCS-A safety-policy docs (AUTOPILOT-RULES.md, AUTOMATION-PERMISSIONS.md, NEXT-ACTION-SELECTOR.md, HANDOFF-RULES.md, ROLE-HIERARCHY.md, PHASE-MODES.md, PROTECTED-FILES.md, CLAUDE.md) NOT modified by this phase (per ARC-8 self-modification HARD BLOCK). Autopilot template set (5 new + 3 extended packet templates + 4 new prompt templates) NOT modified by this phase.
- [ ] ARC-8-RUN-B: Migration 008 runbook NOT modified; N-3 closed; Migration 008 APPLIED at HEAD `189eb1be6ef6304d914671bdaedec44d389cf877`.
- [ ] ARC-8-RUN-B: Loops A/B/C/D all real (no simulated Loop D as in ARC-8-RUN-A). Loop A snapshot completed at HEAD `a427d1c…`. Loop B selector Rule 1 (Finish active closeout) fired first; operator-confirmed Candidate 1 (this back-fill task).
- [ ] ARC-8-RUN-B: Codex Round 1 (Loop C audit / draft review) — pending.
- [ ] ARC-8-RUN-B: Codex Round 2 (Loop D approval-packet review) — pending.
- [ ] ARC-8-RUN-B: Codex Round 3 (Discord-draft sanity check) — pending.
- [ ] ARC-8-RUN-B: Codex Round 4 (optional dry-run / closeout report review) — optional.
- [ ] ARC-8-RUN-B: Loop D approval packet drafted with operator-approval field NOT marked; Channel 1 + 2 Discord drafts NOT auto-published.
- [ ] ARC-8-RUN-B: autopilot runtime remains DORMANT after this phase; no scheduler / webhook / MCP / cron / Discord-bot installed; no autopilot execution authority granted.
- [ ] ARC-8-RUN-B: phase-loop ceiling counter at this phase = 3 of 3 — **the ceiling fires at end of ARC-8-RUN-B; operator direction confirmation required for any further autopilot-driven phase** (per `orchestrator/AUTOPILOT-RULES.md` ARC-8 phase-loop ceiling rule).
- [ ] ARC-8-RUN-B: approval-fatigue queue position at this request = 1 of 2 (this commit-only approval is the single pending request).
- [ ] ARC-8-RUN-B: ARC-8-RUN-B-CLOSEOUT (or ARC-8-CEILING-PAUSE) deferred — both require operator direction confirmation to override the phase-loop ceiling before opening.
- [ ] ARC-8-RUN-B: pending Codex docs-only review (Round 1); pending operator approval to commit; pending separate operator approval to push.

## N-2x Migration 008 Runbook Track

All N-2x phases through ARC-8-RUN-A-CLOSEOUT are CLOSED (or, for N-2r, design-only PASS); ARC-8-RUN-B is IN PROGRESS (DOCS-ONLY first autopilot-driven phase end-to-end). **Migration 008 is APPLIED to production at HEAD `189eb1be6ef6304d914671bdaedec44d389cf877` (Attempt 6 — 2026-05-04, runner exit 0). N-3 is closed. Autopilot runtime remains DORMANT.** The canonical detailed change history lives in `orchestrator/handoffs/N-2-MIGRATION-008-PRODUCTION-PLAN.md` §14; commit truth lives in `git log`. ARC-8 (Controlled Autopilot Builder System) extension is canonical in `orchestrator/AUTOPILOT-RULES.md` ARC-8 section; the autopilot template set is canonical in `orchestrator/handoffs/AUTOPILOT-*.md` and `orchestrator/prompts/AUTOPILOT-LOOP-*.md`. Phase-loop ceiling counter at this phase: 3 of 3 — fires at end of ARC-8-RUN-B; operator direction confirmation required for any further autopilot-driven phase.
...
| ARC-8-RUN-B First autopilot-driven DOCS-ONLY phase end-to-end (back-fill ARC-8-RUN-A-CLOSEOUT stale-tail in 3 status docs; Loops A/B/C/D all real; Codex Rounds 1–3 observed live; phase-loop ceiling 3 of 3 — fires at end of phase) | IN PROGRESS — DOCS-ONLY |
