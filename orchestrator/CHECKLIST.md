...
- [~] **Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS (Codex), ARC-7 in progress (Phase ARC-7 — mode: DOCS-ONLY).**
  ...
  - [~] **ARC-7 — Handoff Packet Templates (Phase ARC-7 — mode: DOCS-ONLY) — IN PROGRESS.**
    - [x] New `orchestrator/HANDOFF-RULES.md` drafted as a safety-policy / control doc establishing the formal handoff packet rules (governance docs only, not channels, not approvals, not canonical, not trading interfaces; forbidden content explicitly enumerated; `CODEX-VERDICT.md` append-only; future-automation governance-only inheritance).
    - [x] Ten packet templates created under `orchestrator/handoffs/`: `PHASE-SNAPSHOT.md`, `CLAUDE-PHASE-PROMPT.md`, `CLAUDE-REPORT-PACKET.md`, `CODEX-REVIEW-PACKET.md`, `CODEX-VERDICT.md` (append-only), `GEMINI-ARCHITECTURE-REVIEW-PACKET.md`, `OPERATOR-APPROVAL-PACKET.md`, `COMMIT-PACKET.md`, `CLOSEOUT-PACKET.md`, `NEXT-PHASE-PROMPT.md`. Each template carries a "What this packet is NOT" disclaimer and explicit non-canonical / non-approval reminders.
    - [x] Per-packet authorship and approval rules documented: Claude DRAFTS but cannot mark approval fields; operator-approval fields are operator-marked only; `CODEX-VERDICT.md` and `GEMINI-ARCHITECTURE-REVIEW-PACKET.md` are operator/Codex/Gemini authored — not Claude-synthesized; future automation may never write `CODEX-VERDICT.md` / `GEMINI-ARCHITECTURE-REVIEW-PACKET.md` / approval fields.
    - [x] Forbidden-content rule enumerated (secrets, env values, API keys, prod-DB content, live Kraken endpoints, live order details, deploy triggers, migration-apply commands, approval-like language not from Victor, automation install/widening instructions). STOP condition triggers on any violation.
    - [x] Registries updated: `orchestrator/PROTECTED-FILES.md` (3 places — added SAFE-table row for HANDOFF-RULES.md plus glob row for `orchestrator/handoffs/**`; safety-policy prose; approval-gate row), `orchestrator/APPROVAL-GATES.md` (2 places — safety-policy note; "approval-gate / phase-mode / next-action-selector / role-hierarchy / automation-permissions / handoff-rules" review bullet), `orchestrator/PHASE-MODES.md` (2 places — DOCS-ONLY allowed-files list now includes HANDOFF-RULES.md and `handoffs/**`; safety-policy docs list).
    - [x] `orchestrator/NEXT-ACTION-SELECTOR.md` master-order updated to insert ARC-7 between N-1 and N-2 (per operator's explicit RED-tier in-session approval); D-5.12f hard-ordering rule extended to require ARC-7 closure as well.
    - [x] ARC-7 status entries added to `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, `orchestrator/NEXT-ACTION.md`.
    - [x] No edits to `bot.js`, `dashboard.js`, `db.js`, `migrations/`, `scripts/`, or `position.json`.
    - [x] No migrations applied. No deploys. No production-DB queries. No live Kraken actions.
    - [x] No installation of Ruflo, Hermes, MCP servers, hooks, schedulers, webhooks, or live transport.
    - [ ] Codex docs-only review of the ARC-7 diff across `HANDOFF-RULES.md`, the ten new templates, the four registry updates (`PROTECTED-FILES.md`, `APPROVAL-GATES.md`, `PHASE-MODES.md`, `NEXT-ACTION-SELECTOR.md`), and the three status-doc updates.
    - [ ] Operator approval to commit ARC-7 docs.
    - [ ] Commit exactly those files after Codex PASS + operator approval.
