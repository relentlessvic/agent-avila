...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-2c closed (`9ae139d`), N-3 attempt-1 2026-05-03 HALTED at HEAD `9ae139d` (DNS / execution-context failure; Migration 008 NOT applied; approval consumed), N-2d closed (`3732721`), N-2e closed (`afe94d1`), N-2f closed (`3af1e44`), N-2g closed (`926eb7f`), N-2h closed (`ea7774d`), N-2i closed (`5ee1dcb`), N-2j closed (`548383b`), N-2k closed (`b2d187d`), N-2l closed (`3138e7f`), **N-3 attempt-2 2026-05-03 HALTED at HEAD `3138e7f`**, N-2m closed (`6b9be1d`), N-2n closed (`8fc53b9`), **fresh Codex N-3 preflight on N-2n at `8fc53b9` returned FAIL** with six required edits (GAP-A through GAP-G; GAP-F + GAP-H sufficient), N-2o closed (`f925ac5`), **fresh Codex N-3 preflight on N-2o at `f925ac5` returned PASS WITH REQUIRED EDITS** with one wording fix (§2 line count "47 lines" → "46 lines"), N-2p in progress (Phase N-2p — mode: DOCS-ONLY).** N-2p is a single-edit Migration 008 line-count correction phase. The fresh Codex N-3 preflight on the N-2o-committed runbook (HEAD `f925ac5`) confirmed all twelve substantive verification checks PASS but flagged one wording fix: §2 said `migrations/008_emergency_audit_log.sql` is "47 lines" but the actual committed file is 46 lines. **N-2p (DOCS-ONLY)** applies that single line-count correction in §2, plus the minimal phase-discipline updates §1 / §9 / §11 / §14 require. **Both Victor approvals (`9ae139d`, `3138e7f`) remain CONSUMED.** §11 condition (1) updated to N-2p. §9 gate-separation chain extended through N-2p. The N-2o GAP-A through GAP-G fixes, the N-2n Railway-attached-deployed-runtime-shell designation, the N-2m same-session / no-context-switch / actual-runner-DNS-path rules, the N-2l parsing-hygiene rule, and the N-2k canonical-source / HALT-on-disagreement rules all remain in force unchanged. No §3 / §4 / §5 / §6 / §7 / §8 / §10 / §12 / §13 / §15 substance changed in N-2p; only §1 / §2 / §9 / §11 / §14 wording updated. Pre-flight remains at eleven checks (i)–(xi); no new check added. The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` values, no hostname / port / URL values added. **Codex's N-3 preflight on N-2o (HEAD `f925ac5`) also flagged a runtime-execution prerequisite (Check D — `package.json` `engines.node` is the non-exact range `">=18.0.0"` and no `.nvmrc` / `.node-version` / Volta config is present) that the operator must resolve at execution time before §4(x)(b) can PASS, but Codex explicitly stated this is NOT a docs-only requirement; N-2p does NOT address it.** **N-3 remains halted/blocked.** No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` / `package.json` / lockfiles / `.nvmrc` / `.node-version` / Volta config. No retry. No rollback. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet. Pending Codex docs-only review and explicit operator approval before any N-2p commit. **N-2p commit-time approval does NOT authorize N-3.**

The next allowed action after this N-2p doc draft is: (a) Codex docs-only review of the runbook diff + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit N-2p; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2p commit, **N-3 remains halted/blocked** behind the five-condition gate, with both prior Victor approvals (`9ae139d` and `3138e7f`) already consumed. Any future N-3 attempt requires fresh Codex N-3 preflight PASS at the new HEAD, fresh Victor approval naming the new HEAD, AND operator-side execution against the approved primary surface — a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container (per N-2n's §3 + §4(vii) gating, with the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with script-existence verification, the §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and the two-context SHA comparison rule), NOT a local `railway run` / `railway shell` session, AND operator-side resolution at execution time of the Check D Node-pin gap that N-2p does not address. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2p commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h / N-2i / N-2j / N-2k / N-2l / N-2m / N-2n / N-2o / N-2p / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
...
> **N-2p Codex docs-only review of the §2 line-count correction runbook update, then explicit operator approval before any N-2p commit; N-3 remains halted/blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2p, fresh Codex N-3 preflight PASS, fresh Victor approval naming a NEW HEAD — both `9ae139d` and `3138e7f` approvals are consumed, all eleven pre-flight checks (i)–(xi) PASS at execution time from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container per the N-2n §3 + §4(vii) gating with the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with script-existence verification, §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and two-context SHA comparison rule, target confirmation without exposing secrets, AND operator-side resolution at execution time of the Check D Node-pin gap that N-2p does not address).**
...
- Before applying Migration 008: follow the current N-2p runbook gate — fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD (note: both prior approvals at `9ae139d` and `3138e7f` are consumed), all eleven §4 pre-flight checks (i)–(xi) PASS at execution time **from a Railway-attached deployed-runtime shell into the existing `agent-avila-dashboard` container** (the approved primary execution surface per N-2n; local Railway CLI run-mode / shell-mode are explicitly REJECTED for internal-hostname `DATABASE_URL`) — including the N-2o-tightened deployed-runtime-shell availability HALT branches, deployed-runtime working-directory verification, approved-runner-invocation form with `scripts/run-migrations.js` existence verification at the expected path inside the deployed container before invocation, the §4(x)(a) git-may-be-absent fallback to non-secret Railway deploy-metadata, and the two-context SHA comparison rule — operator-side resolution of the Check D Node-pin gap (Codex flagged at HEAD `f925ac5`; N-2p does not address it), target confirmation without exposing secrets, then the full §6 verification after runner exit.
