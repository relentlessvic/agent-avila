...
> **N-2c Codex docs-only review of the corrected runbook; then explicit operator approval before any N-2c commit; N-3 remains hard-blocked behind separate explicit Victor approval that names the exact current HEAD.**

ARC-7 closed (`8266db2`). N-2 returned Codex PASS WITH REQUIRED EDITS (conversation-only). N-2b committed at `e6c9189` (runbook artifact + 5 Codex tightenings + 4 cleanup rounds for full HANDOFF-RULES.md prose-only compliance). **N-2c (DOCS-ONLY)** corrects a stale-pinned-HEAD finding from the Codex N-3 preflight review: the runbook's pre-flight check (i) still pinned HEAD to `8266db2…` (pre-N-2b), but the runbook is committed at `e6c9189`. N-2c replaces the hardcoded HEAD pin with an approval-time exact-HEAD-naming rule, avoiding the self-staling commit loop. The current draft touches exactly the runbook artifact (`orchestrator/handoffs/N-2-MIGRATION-008-PRODUCTION-PLAN.md`) plus minimal status entries in `orchestrator/STATUS.md`, `orchestrator/CHECKLIST.md`, and `orchestrator/NEXT-ACTION.md`. No safety-policy doc is changed in N-2c. Next sequence: Codex docs-only review of the N-2c runbook diff + the three status-doc edits; on Codex PASS, ask the operator for explicit approval to commit; only after that approval, stage by name and commit. After N-2c commit, **N-3 (Migration 008 production application, mode PRODUCTION ACTION) remains hard-blocked** until: (a) fresh Codex N-3 preflight PASS on the corrected runbook; (b) explicit Victor in-session production-action approval **naming the exact current HEAD hash returned by `git rev-parse HEAD` immediately before approval** (per the runbook's exact-HEAD-naming requirement); (c) all pre-flight checks pass at execution time; (d) operator confirms target Railway service and production `DATABASE_URL` without exposing secret. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2c commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-3 are all handled per the master order. The two pre-existing alternative candidates remain (operator may explicitly change the master order):
