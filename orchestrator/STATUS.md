...
**Hands-Free ARC track — ARC-0 PASS, ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-2c closed (`9ae139d`), N-3 attempt 2026-05-03 HALTED (runner exit 1; `getaddrinfo ENOTFOUND` for Railway-internal Postgres hostname; no SQL executed; production-DB unchanged; Migration 008 NOT applied; prior Victor approval consumed), N-2d closed (`3732721`), N-2e closed (`afe94d1`), N-2f closed (`3af1e44`), N-2g in progress (Phase N-2g — mode: DOCS-ONLY).** N-2g is a Codex-required N-3 preflight runtime-identity + DB-baseline + descriptive-state tightening phase: the fresh N-3 preflight review on the N-2f-committed runbook (HEAD `3af1e44`) returned FAIL with four required edits — (a) §4(x) under-specified (permissive examples would let near-HEAD / short-hash / branch-name / service-name / "latest deploy" identity pass instead of an exact full-hash comparison); (b) §4(xi) under-specified (no exact query target named, no immediate-before-invocation timing, no link to §7 HALT for no-op runner output); (c) §8 mitigation chains inherited the under-specified (x) and (xi) plus the wrong-target row cited only §4(vi) and not §4(vii)(d); (d) §14 / phase label / status chain stale (runbook header still read "Phase: N-2b", §1 status chain stopped at `afe94d1`, §14 N-2f tail still said "Pending commit" even though N-2f committed at `3af1e44`). N-2g applies all 4 Codex-required edits: tightens §4(x) to require exact full-hash comparison (local: `git rev-parse HEAD`; Railway-attached: deployed-revision / source-identity surface; near-HEAD / short-hash / branch / service / "latest deploy" explicitly NOT sufficient); tightens §4(xi) to name `schema_migrations.version` exact query target, require immediate-before-invocation timing after §4(viii), and link no-op runner output to §7 HALT; updates §8 mitigation chains; updates runbook header phase label to "N-2g", extends §1 status chain to include `3af1e44` (N-2f), corrects §14 N-2f entry tail. Pre-flight remains at eleven checks (i)–(xi); no new check added in N-2g. The runbook remains fully prose-only and HANDOFF-RULES.md compliant — no literal Railway runner commands, no `DATABASE_URL` values, no hostname / port / URL values added. **Migration 008 remains NOT applied to production.** **No production-DB state changed during the N-3 attempt — the runner exited at the connection step before any transaction was opened. No rollback is needed and none is authorized.** N-3 remains hard-blocked pending: (1) Codex docs-only PASS on this N-2g update; (2) fresh Codex N-3 preflight PASS on the updated runbook; (3) fresh Victor explicit in-session production-action approval naming the exact current HEAD; (4) all eleven pre-flight checks (i)–(xi) PASS at execution time; (5) target Railway service / production database confirmation without exposing secrets. **No code, no migrations applied, no deploys, no Railway commands invoked by Claude, no production-DB queries by Claude, no env / secret access, no `MANUAL_LIVE_ARMED` change, no live Kraken actions, no edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json` during N-2g.** Pending Codex docs-only review of this runbook update + status-doc edits, then explicit operator approval before any N-2g commit. N-2b is closed at `e6c9189`; N-2c is closed at `9ae139d`; N-2d is closed at `3732721`; N-2e is closed at `afe94d1`; N-2f is closed at `3af1e44`; **N-2g is the only active docs-only correction phase.** N-3 remains hard-blocked behind the fresh five-condition gate (Codex docs-only PASS on N-2g, fresh Codex N-3 preflight PASS on the updated runbook, fresh Victor in-session production-action approval naming the exact current HEAD, all eleven pre-flight checks (i)–(xi) PASS at execution time, and target Railway service / production database confirmation without exposing secrets).
...
- **Migration 008** (`emergency_audit_log`) — file landed in repo at `migrations/008_emergency_audit_log.sql` via D-5.12c (`4ae3689`). **NOT YET APPLIED to production.** Application requires a separate explicit operator authorization gate. The file's header explicitly warns against running it via `scripts/run-migrations.js` without operator authorization. Before any future application, follow the current N-2g runbook gate: fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven §4 pre-flight checks (i)–(xi) PASS at execution time, and target confirmation without exposing secrets.
...
> **N-2g Codex docs-only review of the N-3 preflight runtime-identity + DB-baseline + descriptive-state tightening runbook update; then explicit operator approval before any N-2g commit; N-3 remains hard-blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2g, fresh Codex N-3 preflight PASS on the updated runbook, fresh Victor approval naming exact current HEAD, all eleven pre-flight checks PASS, target confirmation without exposing secrets).**

N-2c closed (`9ae139d`). On 2026-05-03, a single N-3 attempt was made and HALTED before any SQL executed: the runner exited 1 with `getaddrinfo ENOTFOUND` for the Railway-internal Postgres hostname, because the execution context (operator's local machine) cannot resolve Railway's internal-network DNS. No `applying` log line appeared, no `BEGIN` was issued, no SQL was executed. Production-DB state is unchanged; Migration 008 is still NOT applied; no partial state; no rollback needed and none authorized. The prior Victor approval was consumed by the failed attempt. **N-2d (DOCS-ONLY)** updates the runbook to add: (a) an §1.1 N-3 attempt history; (b) a §3 Railway-attached execution-context requirement; (c) a new pre-flight check (vii) connectivity preflight; (d) §7 HALT-on-unreachable-target and HALT-on-connection-failure stop conditions plus a do-not-retry-automatically rule; (e) two new risk-table rows (P0 hostname-resolution; P1 consumed-approval). The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` examples, no hostname/port/URL values added. N-2d is closed at `3732721`; N-2e is closed at `afe94d1`; N-2f is closed at `3af1e44`. The fresh Codex N-3 preflight review on the N-2f-committed runbook (HEAD `3af1e44`) returned FAIL with four required edits, driving **N-2g (DOCS-ONLY)** as the current N-3 preflight runtime-identity + DB-baseline + descriptive-state tightening phase. N-2g applies all 4 Codex-required edits to the runbook (tightened §4(x) full-hash comparison; tightened §4(xi) `schema_migrations.version` query target + immediate-before-invocation timing + §7 no-op linkage; §8 mitigation chain updates; §14 / phase label / status chain stale-state corrections). Pre-flight remains at eleven checks (i)–(xi); no new check added. The current N-2g draft touches exactly the runbook plus minimal status entries in this file, `CHECKLIST.md`, and `NEXT-ACTION.md`. No safety-policy doc is changed in N-2g. Next sequence: Codex docs-only review of the N-2g diff + the three status-doc edits; on Codex PASS, ask the operator for explicit approval to commit; only after that approval, stage by name and commit. After N-2g commit, **N-3 remains hard-blocked** until all five gate conditions are simultaneously satisfied with a fresh Codex N-3 preflight PASS on the now-updated runbook and a fresh Victor in-session production-action approval naming the exact current HEAD at retry time. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2g commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-3 are all handled per the master order. The two pre-existing alternative candidates remain (operator may explicitly change the master order):
