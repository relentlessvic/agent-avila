...
**Hands-Free ARC track — ARC-1 closed (`95157ae`), ARC-2 closed (`3b53223`), ARC-3 closed (`9b5093f`), ARC-4 closed (`afd5930`), ARC-5 closed (`698e0d3`), ARC-6 closed (`9eff3c9`), N-1 PASS, N-1 closeout sync closed (`4d472e2`), N-1.5 design review PASS, ARC-7 closed (`8266db2`), N-2 design review PASS WITH REQUIRED EDITS, N-2b closed (`e6c9189`), N-2c closed (`9ae139d`), N-3 attempt 2026-05-03 HALTED (runner exit 1; `getaddrinfo ENOTFOUND` for Railway-internal Postgres hostname; no SQL executed; production-DB unchanged; Migration 008 NOT applied; prior Victor approval consumed), N-2d closed (`3732721`), N-2e closed (`afe94d1`), N-2f closed (`3af1e44`), N-2g closed (`926eb7f`), N-2h closed (`ea7774d`), N-2i in progress (Phase N-2i — mode: DOCS-ONLY).** ARC-1 through ARC-7 closed in sequence. N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h established, stabilized, connectivity-preflight-extended, N-3-preflight-tightened, DDL-baseline + rollback-blocker-tightened, runtime-identity + DB-baseline-tightened, and §14 stale-tail-loop-fixed the production-plan runbook. The single N-3 attempt on 2026-05-03 HALTED before any SQL executed because the runner was invoked from a non-Railway-attached execution context. Production-DB state is unchanged. Migration 008 is still NOT applied. The prior Victor approval was consumed by the failed attempt and cannot be reused. The fresh Codex N-3 preflight review on the N-2h-committed runbook (HEAD `ea7774d`) returned FAIL with four findings — (a) §1 Status block's self-hash chain didn't include N-2h's own commit hash because §1 had been drafted pre-commit (the §14 stale-tail loop simply moved from §14 to §1); (b) §14 preamble's "auto-stay-current" claim was incorrect; (c) §8 P1 stale-Railway-CLI row contained literal `railway run` / `railway shell` strings, a HANDOFF-RULES.md leak in the runbook since N-2e; (d) §4(x) Node/runtime/dependency drift mitigation gap — `git rev-parse HEAD` only catches source-file identity, not Node binary version or `node_modules` content. **N-2i (DOCS-ONLY)** breaks the §1 self-hash loop at its root: removes self-referential committed-HEAD tracking from §1 (replaces hash chain with a prose phase chain and explicitly defers commit-hash truth to `git log` / `git show`), qualifies the §14 preamble's "auto-stay-current" wording, replaces literal Railway command-string references with generic prose, and tightens §4(x) from a single full-hash check into a three-component identity check (source revision + Node version + dependency / build identity, all three must PASS independently). §11 condition (1) updated to N-2i. §9 gate-separation chain extended through N-2i. No §4(i)–(ix) / §4(xi) / §5 / §6 / §7 / §8 risk-table substance other than the runner-version-drift row is changed; §4(x) is strengthened, not weakened. Pre-flight remains at eleven checks (i)–(xi); no new check added. The runbook remains fully prose-only — no literal Railway runner commands, no `DATABASE_URL` values, no hostname/port/URL values added. **N-3 remains hard-blocked** behind a fresh five-condition gate: (1) Codex docs-only PASS on this N-2i update; (2) fresh Codex N-3 preflight PASS on the now-updated runbook; (3) fresh Victor explicit in-session production-action approval naming the exact current HEAD at retry time; (4) all eleven pre-flight checks (i)–(xi) PASS at execution time; (5) target Railway service / production database confirmation without exposing secrets. **No edits to `bot.js` / `dashboard.js` / `db.js` / `scripts/` / `migrations/` / `position.json`. No migration applied. No deploy. No production-DB query. No Railway command. No live Kraken action. No env / secret read or write. No `MANUAL_LIVE_ARMED` change. No commit yet.** Pending Codex docs-only review and explicit operator approval before any N-2i commit.

The next allowed action after this N-2i doc draft is: (a) Codex docs-only review of the runbook diff + the three minimal status-doc edits; (b) on Codex PASS (or PASS WITH REQUIRED EDITS, after applying any wording fixes), explicit operator approval to commit N-2i; (c) commit exactly those four paths — never stage or modify any HARD BLOCK file. After N-2i commit, **N-3 remains hard-blocked** until all five gate conditions are simultaneously satisfied. Per `orchestrator/APPROVAL-GATES.md` gate 4 separation rule, **N-2i commit-time approval does NOT authorize N-3.** **D-5.12f (live SELL_ALL design-only review) remains hard-blocked** until ARC-7 / N-2 / N-2b / N-2c / N-2d / N-2e / N-2f / N-2g / N-2h / N-2i / N-3 are all handled per the master order, unless the operator explicitly changes the master order again.
...
> **N-2i Codex docs-only review of the structural self-hash-loop removal + runtime-drift tightening runbook update, then explicit operator approval before any N-2i commit; N-3 remains hard-blocked behind a fresh five-condition gate (Codex docs-only PASS on N-2i, fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven pre-flight checks (i)–(xi) PASS, target confirmation without exposing secrets).**
...
- Before applying Migration 008: follow the current N-2i runbook gate — fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven §4 pre-flight checks (i)–(xi) PASS at execution time (including the §4(x) three-component runtime-source identity check — source revision + Node version + dependency / build identity), target confirmation without exposing secrets, then the full §6 verification after runner exit.
...
For **Migration 008 production-application planning** (when authorized): the operator-driven sequence is — (1) satisfy the current N-2i runbook gate, including fresh Codex N-3 preflight PASS, fresh Victor approval naming exact current HEAD, all eleven §4 pre-flight checks (i)–(xi) PASS at execution time (including the §4(x) three-component runtime-source identity check), and target confirmation without exposing secrets; (2) apply Migration 008 only under that explicit production-action approval; (3) complete the full §6 post-application verification; (4) document the application timestamp in `STATUS.md`. After application, D-5.12d/e production deployment is unblocked (still requires its own explicit operator authorization).
